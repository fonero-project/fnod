...
const (
	...
	unusableFlags = fnojson.UFWebsocketOnly | fnojson.UFNotification
)

var (
	fnodHomeDir          = fnoutil.AppDataDir("fnod", false)
	fnoctlHomeDir        = fnoutil.AppDataDir("fnoctl", false)
	fnowalletHomeDir     = fnoutil.AppDataDir("fnowallet", false)
	defaultConfigFile      = filepath.Join(fnoctlHomeDir, "fnoctl.conf")
	defaultRPCServer       = "localhost"
	defaultWalletRPCServer = "localhost"
	defaultRPCCertFile     = filepath.Join(fnodHomeDir, "rpc.cert")
...
func listCommands() {
	...
	cmdMethods := fnojson.RegisteredCmdMethods()
	categorized := make([][]string, numCategories)
	for _, method := range cmdMethods {
		flags, err := fnojson.MethodUsageFlags(method)
		if err != nil {
		...
		usage, err := fnojson.MethodUsageText(method)
		if err != nil {
			// This should never happen since the method was just
			// returned from the package, but be safe.
			continue
		...
		if flags&fnojson.UFWalletOnly != 0 {
...
// config defines the configuration options for fnoctl.
//
...
// For this it tries to read the fnod config file at its default path, and extract
// the RPC user and password from it.
func createDefaultConfigFile(destinationPath string) error {
	// Nothing to do when there is no existing fnod conf file at the default
	// path to extract the details from.
	fnodConfigPath := filepath.Join(fnodHomeDir, "fnod.conf")
	if !fileExists(fnodConfigPath) {
		return nil
	}

	// Read fnod.conf from its default path
	fnodConfigFile, err := os.Open(fnodConfigPath)
	if err != nil {
		return err
	}
	defer fnodConfigFile.Close()
	content, err := ioutil.ReadAll(fnodConfigFile)
	if err != nil {
		return err
	}
