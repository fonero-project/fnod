...
func main() {
	...
	method := args[0]
	usageFlags, err := fnojson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := fnojson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// fnojson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(fnojson.Error); ok {
		...
		// The error is not a fnojson.Error and this really should not
	...
	marshalledJSON, err := fnojson.MarshalCmd("1.0", 1, cmd)
	if err != nil {
