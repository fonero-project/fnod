...
fnod provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, fnod supports a hybrid,
...
While it is easier to only run as a client, it is more beneficial to the Fonero
...
Configuring fnod as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the fnod command
line or in the fnod configuration file.  Typically the Tor proxy address will be
...
By default, fnod assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since fnod will act as the server.
...
```bash
$ ./fnod --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
fnod provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the fnod command line or in the fnod
...
```bash
$ ./fnod --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
fnod provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./fnod --proxy=127.0.0.1:9050 --torisolation
```
