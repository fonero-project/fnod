...
func (k *ExtendedKey) Child(i uint32) (*ExtendedKey, error) {
	...
	parentFP := fnoutil.Hash160(k.pubKeyBytes())[:4]
...
// ECPubKey converts the extended key to a fnoec public key and returns it.
func (k *ExtendedKey) ECPubKey() (*secp256k1.PublicKey, error) {
	return secp256k1.ParsePubKey(k.pubKeyBytes())
}

// ECPrivKey converts the extended key to a fnoec private key and returns it.
...
// Address converts the extended key to a standard Fonero pay-to-pubkey-hash
// address for the passed network.
func (k *ExtendedKey) Address(net *chaincfg.Params) (*fnoutil.AddressPubKeyHash, error) {
	pkHash := fnoutil.Hash160(k.pubKeyBytes())
	return fnoutil.NewAddressPubKeyHash(pkHash, net, fnoec.STEcdsaSecp256k1)
}
