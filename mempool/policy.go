...
const (
	...
	DefaultMinRelayTxFee = fnoutil.Amount(1e4)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee fnoutil.Amount) int64 {
	...
	if minFee < 0 || minFee > fnoutil.MaxAmount {
		minFee = fnoutil.MaxAmount
	}
...
func checkInputsStandard(tx *fnoutil.Tx, txType stake.TxType, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee fnoutil.Amount) bool {
...
func checkTransactionStandard(tx *fnoutil.Tx, txType stake.TxType, height int64,
	medianTime time.Time, minRelayTxFee fnoutil.Amount,
