...
func TestCalcMinRequiredTxRelayFee(t *testing.T) {
	tests := []struct {
		...
		relayFee fnoutil.Amount // minimum relay transaction fee.
...
func TestDust(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wire.TxOut{Value: fnoutil.MaxAmount, Version: 0, PkScript: pkScript},
			fnoutil.MaxAmount,
			false,
		},
		{
...
func TestCheckTransactionStandard(t *testing.T) {
	...
	addr, err := fnoutil.NewAddressPubKeyHash(addrHash[:],
		&chaincfg.RegNetParams, fnoec.STEcdsaSecp256k1)
	if err != nil {
