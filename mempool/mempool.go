...
import (
	...
	"github.com/fonero-project/fnod/blockchain/indexers"
	...
	"github.com/fonero-project/fnod/mining"
...
const (
	...
	// FNO/kB, this results in a maximum allowed high fee of 1 FNO/kB.
...
type Config struct {
	...
	FetchUtxoView func(*fnoutil.Tx, bool) (*blockchain.UtxoViewpoint, error)

	// BlockByHash defines the function use to fetch the block identified
	// by the given hash.
	BlockByHash func(*chainhash.Hash) (*fnoutil.Block, error)
	...
	// utxo view.
	CalcSequenceLock func(*fnoutil.Tx, *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error)
...
type Policy struct {
	...
	// MinRelayTxFee defines the minimum transaction fee in FNO/kB to be
	// considered a non-zero fee.
	MinRelayTxFee fnoutil.Amount
...
type TxPool struct {
	...
	orphans       map[chainhash.Hash]*fnoutil.Tx
	orphansByPrev map[wire.OutPoint]map[chainhash.Hash]*fnoutil.Tx
	outpoints     map[wire.OutPoint]*fnoutil.Tx
...
func (mp *TxPool) insertVote(ssgen *fnoutil.Tx) error {
	msgTx := ssgen.MsgTx()
	...
	vote := fnoutil.IsFlagSet16(voteBits, fnoutil.BlockValid)
...
func (mp *TxPool) removeOrphan(tx *fnoutil.Tx, removeRedeemers bool) {
	txHash := tx.Hash()
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveOrphan(tx *fnoutil.Tx) {
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addOrphan(tx *fnoutil.Tx) {
	...
	for _, txIn := range tx.MsgTx().TxIn {
		if _, exists := mp.orphansByPrev[txIn.PreviousOutPoint]; !exists {
			mp.orphansByPrev[txIn.PreviousOutPoint] =
				make(map[chainhash.Hash]*fnoutil.Tx)
		}
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAddOrphan(tx *fnoutil.Tx) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeOrphanDoubleSpends(tx *fnoutil.Tx) {
	msgTx := tx.MsgTx()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeTransaction(tx *fnoutil.Tx, removeRedeemers bool) {
	txHash := tx.Hash()
	if removeRedeemers {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveDoubleSpends(tx *fnoutil.Tx) {
	// Protect concurrent access.
	mp.mtx.Lock()
	for _, txIn := range tx.MsgTx().TxIn {
...
func (mp *TxPool) addTransaction(utxoView *blockchain.UtxoViewpoint,
	tx *fnoutil.Tx, txType stake.TxType, height int64, fee int64) {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *fnoutil.Tx, txType stake.TxType) error {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) fetchInputUtxos(tx *fnoutil.Tx) (*blockchain.UtxoViewpoint, error) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchTransaction(txHash *chainhash.Hash) (*fnoutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.RLock()
...
// It should also set the fnoutil tree type for the tx as well.
func (mp *TxPool) maybeAcceptTransaction(tx *fnoutil.Tx, isNew, rateLimit, allowHighFees, rejectDupOrphans bool) ([]*chainhash.Hash, error) {
	msgTx := tx.MsgTx()
	txHash := tx.Hash()
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptTransaction(tx *fnoutil.Tx, isNew, rateLimit bool) ([]*chainhash.Hash, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(acceptedTx *fnoutil.Tx) []*fnoutil.Tx {
	var acceptedTxns []*fnoutil.Tx
	...
	for processList.Len() > 0 {
		...
		processItem := firstElement.(*fnoutil.Tx)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) ProcessTransaction(tx *fnoutil.Tx, allowOrphan, rateLimit, allowHighFees bool) ([]*fnoutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
	defer mp.mtx.Unlock()
	var err error
	...
	if len(missingParents) == 0 {
		...
		acceptedTxs := make([]*fnoutil.Tx, len(newTxs)+1)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RawMempoolVerbose(filterType *stake.TxType) map[string]*fnojson.GetRawMempoolVerboseResult {
	mp.mtx.RLock()
	defer mp.mtx.RUnlock()

	result := make(map[string]*fnojson.GetRawMempoolVerboseResult,
		len(mp.pool))
	bestHeight := mp.cfg.BestHeight()
	...
	for _, desc := range mp.pool {
		...
		mpd := &fnojson.GetRawMempoolVerboseResult{
			Size:             int32(tx.MsgTx().SerializeSize()),
			Fee:              fnoutil.Amount(desc.Fee).ToCoin(),
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		orphans:       make(map[chainhash.Hash]*fnoutil.Tx),
		orphansByPrev: make(map[wire.OutPoint]map[chainhash.Hash]*fnoutil.Tx),
		outpoints:     make(map[wire.OutPoint]*fnoutil.Tx),
