...
type fakeChain struct {
	...
	blocks         map[chainhash.Hash]*fnoutil.Block
...
func (s *fakeChain) FetchUtxoView(tx *fnoutil.Tx, treeValid bool) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) BlockByHash(hash *chainhash.Hash) (*fnoutil.Block, error) {
	s.RLock()
...
func (s *fakeChain) AddBlock(block *fnoutil.Block) {
	s.Lock()
...
func (s *fakeChain) CalcSequenceLock(tx *fnoutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
...
func (s *fakeChain) AddFakeUtxoMedianTime(tx *fnoutil.Tx, txOutIdx uint32, medianTime time.Time) {
	s.Lock()
...
// transactions.
func txOutToSpendableOut(tx *fnoutil.Tx, outputNum uint32, tree int8) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum, Tree: tree},
		amount:   fnoutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
// passed in.
func (p *poolHarness) GetScript(addr fnoutil.Address) ([]byte, error) {
...
func (p *poolHarness) GetKey(addr fnoutil.Address) (chainec.PrivateKey, bool, error) {
	return p.signKey, true, nil
}

// AddFakeUTXO creates a fake mined uxto for the provided transaction.
func (p *poolHarness) AddFakeUTXO(tx *fnoutil.Tx, blockHeight int64) {
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int64, numOutputs uint32) (*fnoutil.Tx, error) {
...
// testing.
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput, numOutputs uint32, mungers ...func(*wire.MsgTx)) (*fnoutil.Tx, error) {
	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput fnoutil.Amount
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*fnoutil.Tx, error) {
	txChain := make([]*fnoutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, fnoutil.NewTx(tx))
...
// output.
func (p *poolHarness) CreateTx(out spendableOutput) (*fnoutil.Tx, error) {
...
func (p *poolHarness) CreateTicketPurchase(sourceTx *fnoutil.Tx, cost int64) (*fnoutil.Tx, error) {
	ticketfee := fnoutil.Amount(singleInputTicketSize)
	ticketPrice := fnoutil.Amount(cost)
	...
	change := fnoutil.Amount(sourceTx.MsgTx().TxOut[0].Value) -
...
func (p *poolHarness) CreateVote(ticket *fnoutil.Tx) (*fnoutil.Tx, error) {
	...
	signedScript, err := txscript.SignTxOutput(p.chainParams, vote, inputToSign,
		...
		p, vote.TxIn[inputToSign].SignatureScript, fnoec.STEcdsaSecp256k1)
	if err != nil {
		return nil, err
	}
	...
	return fnoutil.NewTx(vote), nil
}

// CreateRevocation creates a revocation using the provided ticket.
func (p *poolHarness) CreateRevocation(ticket *fnoutil.Tx) (*fnoutil.Tx, error) {
	...
	signedScript, err := txscript.SignTxOutput(p.chainParams, revocation, inputToSign,
		redeemTicketScript, txscript.SigHashAll, p,
		p, revocation.TxIn[inputToSign].SignatureScript, fnoec.STEcdsaSecp256k1)
	if err != nil {
		return nil, err
	}
	...
	return fnoutil.NewTx(revocation), nil
}
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	payPubKeyAddr, err := fnoutil.NewAddressSecpPubKey(pubKeyBytes,
		chainParams)
	if err != nil {
		return nil, nil, err
	}
	...
	chain := &fakeChain{
		...
		blocks:      make(map[chainhash.Hash]*fnoutil.Block),
...
func testPoolMembership(tc *testContext, tx *fnoutil.Tx, inOrphanPool, inTxPool bool) {
	txHash := tx.Hash()
...
func TestExpirationPruning(t *testing.T) {
	...
	expiringTxns := make([]*fnoutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
...
func TestBasicOrphanRemoval(t *testing.T) {
	...
	nonChainedOrphanTx, err := harness.CreateSignedTx([]spendableOutput{{
		amount:   fnoutil.Amount(5000000000),
...
func TestSequenceLockAcceptance(t *testing.T) {
	...
	for _, acceptSeqLocks := range []bool{false, true} {
		...
		for i, test := range tests {
			...
			inputTx := fnoutil.NewTx(inputMsgTx)
