...
var (
	...
	// data.
	gbtCoinbaseAux = &fnojson.GetBlockTemplateResultAux{
...
var (
	...
	ErrRPCUnimplemented = &fnojson.RPCError{
	...
	ErrRPCNoWallet = &fnojson.RPCError{
		Code:    fnojson.ErrRPCNoWallet,
...
// list of commands that we recognize, but for which fnod has no support because
// it lacks support for wallet functionality. For these commands the user
// should ask a connected instance of fnowallet.
...
// not needed.
func rpcInternalError(errStr, context string) *fnojson.RPCError {
	logStr := errStr
	if context != "" {
		logStr = context + ": " + errStr
	}
	rpcsLog.Error(logStr)
	return fnojson.NewRPCError(fnojson.ErrRPCInternal.Code, errStr)
}
...
func rpcInvalidError(fmtStr string, args ...interface{}) *fnojson.RPCError {
	return fnojson.NewRPCError(fnojson.ErrRPCInvalidParameter,
...
func rpcDeserializationError(fmtStr string, args ...interface{}) *fnojson.RPCError {
	return fnojson.NewRPCError(fnojson.ErrRPCDeserialization,
		fmt.Sprintf(fmtStr, args...))
}

// rpcRuleError is a convenience function to convert a
// rule error to an RPC error with the appropriate code set.
func rpcRuleError(fmtStr string, args ...interface{}) *fnojson.RPCError {
	return fnojson.NewRPCError(fnojson.ErrRPCMisc,
		fmt.Sprintf(fmtStr, args...))
}
...
func rpcDuplicateTxError(fmtStr string, args ...interface{}) *fnojson.RPCError {
	return fnojson.NewRPCError(fnojson.ErrRPCDuplicateTx,
		fmt.Sprintf(fmtStr, args...))
}
...
// context parameter is only used in the log message and may be empty if it's
// not needed.
func rpcAddressKeyError(fmtStr string, args ...interface{}) *fnojson.RPCError {
	return fnojson.NewRPCError(fnojson.ErrRPCInvalidAddressOrKey,
		fmt.Sprintf(fmtStr, args...))
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *fnojson.RPCError {
	return fnojson.NewRPCError(fnojson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *fnojson.RPCError {
	return fnojson.NewRPCError(fnojson.ErrRPCNoTxInfo,
...
func rpcMiscError(message string) *fnojson.RPCError {
	return fnojson.NewRPCError(fnojson.ErrRPCMisc, message)
}
...
// These commands will be implemented in fnowallet.
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.CreateRawTransactionCmd)
	...
	for _, input := range c.Inputs {
		...
		if input.Amount > 0 {
			amt, err := fnoutil.NewAmount(input.Amount)
			if err != nil {
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount > fnoutil.MaxAmount {
			return nil, rpcInvalidError("Invalid amount: 0 >= %v "+
				"> %v", amount, fnoutil.MaxAmount)
		}

		// Decode the provided address.
		addr, err := fnoutil.DecodeAddress(encodedAddr)
		if err != nil {
		...
		atomic, err := fnoutil.NewAmount(amount)
		if err != nil {
			return nil, rpcInternalError(err.Error(),
				"New amount")
		}
...
func handleCreateRawSStx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.CreateRawSStxCmd)
	...
	for i, cout := range c.COuts {
		...
		addr, err := fnoutil.DecodeAddress(cout.Addr)
		if err != nil {
			return nil, rpcAddressKeyError("Could not decode "+
				"address: %v", err)
		}
		...
		pkScript, err := txscript.GenerateSStxAddrPush(addr,
			fnoutil.Amount(amountsCommitted[i]), 0x0000)
		if err != nil {
			return nil, rpcInternalError(err.Error(),
		...
		// Ensure amount is in the valid range for monetary amounts.
		if cout.ChangeAmt < 0 || cout.ChangeAmt > fnoutil.MaxAmount {
			return nil, rpcInvalidError("Invalid change amount: 0 "+
				"> %v > %v", cout.ChangeAmt, fnoutil.MaxAmount)
		}

		// Decode the provided address.
		addr, err = fnoutil.DecodeAddress(cout.ChangeAddr)
		if err != nil {
			return nil, rpcAddressKeyError("Wrong network: %v",
				addr)
		}
...
func handleCreateRawSSRtx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.CreateRawSSRtxCmd)
	...
	var feeAmt fnoutil.Amount
	if c.Fee != nil {
		var err error
		feeAmt, err = fnoutil.NewAmount(*c.Fee)
		if err != nil {
	...
	for i, ssrtxPkh := range ssrtxPkhs {
		// Ensure amount is in the valid range for monetary amounts.
		if sstxAmts[i] <= 0 || sstxAmts[i] > fnoutil.MaxAmount {
			return nil, rpcInvalidError("Invalid SSTx amount: 0 >="+
				" %v > %v", sstxAmts[i] <= 0, fnoutil.MaxAmount)
		}
...
// transaction.
func createVinList(mtx *wire.MsgTx) []fnojson.Vin {
	// Coinbase transactions only have a single txin by definition.
	vinList := make([]fnojson.Vin, len(mtx.TxIn))
	if blockchain.IsCoinBaseTx(mtx) {
		txIn := mtx.TxIn[0]
		vinEntry := &vinList[0]
		vinEntry.Coinbase = hex.EncodeToString(txIn.SignatureScript)
		vinEntry.Sequence = txIn.Sequence
		vinEntry.AmountIn = fnoutil.Amount(txIn.ValueIn).ToCoin()
	...
	for i, txIn := range mtx.TxIn {
		...
		vinEntry.BlockHeight = txIn.BlockHeight
		vinEntry.BlockIndex = txIn.BlockIndex
		vinEntry.ScriptSig = &fnojson.ScriptSig{
			Asm: disbuf,
...
// transaction.
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []fnojson.Vout {

	txType := stake.DetermineTxType(mtx)
	voutList := make([]fnojson.Vout, 0, len(mtx.TxOut))
	for i, v := range mtx.TxOut {
		...
		var vout fnojson.Vout
		voutSPK := &vout.ScriptPubKey
		vout.N = uint32(i)
		vout.Value = fnoutil.Amount(v.Value).ToCoin()
		...
		if commitAmt != nil {
			voutSPK.CommitAmt = fnojson.Float64(commitAmt.ToCoin())
		}
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx, txHash string, blkIdx uint32, blkHeader *wire.BlockHeader, blkHash string, blkHeight int64, confirmations int64) (*fnojson.TxRawResult, error) {
	mtxHex, err := messageToHex(mtx)
	if err != nil {
		return nil, err
	}
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.DecodeRawTransactionCmd)
	...
	txReply := fnojson.TxRawDecodeResult{
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	p2sh, err := fnoutil.NewAddressScriptHash(script, s.server.chainParams)
	if err != nil {
		return nil, rpcInternalError(err.Error(),
	...
	reply := fnojson.DecodeScriptResult{
		Asm:       disbuf,
...
func handleEstimateSmartFee(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.EstimateSmartFeeCmd)

	mode := fnojson.EstimateSmartFeeConservative
	if c.Mode != nil {
		mode = *c.Mode
	}

	if mode != fnojson.EstimateSmartFeeConservative {
...
func handleEstimateStakeDiff(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.EstimateStakeDiffCmd)
	...
	if c.Tickets != nil {
		...
		userEstFlt := fnoutil.Amount(userEst).ToCoin()
		userEstFltPtr = &userEstFlt
	}

	return &fnojson.EstimateStakeDiffResult{
		Min:      fnoutil.Amount(min).ToCoin(),
		Max:      fnoutil.Amount(max).ToCoin(),
		Expected: fnoutil.Amount(expected).ToCoin(),
		User:     userEstFltPtr,
	}, nil
}
...
func handleExistsAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*fnojson.ExistsAddressCmd)

	// Attempt to decode the supplied address.
	addr, err := fnoutil.DecodeAddress(c.Address)
	if err != nil {
...
func handleExistsAddresses(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*fnojson.ExistsAddressesCmd)
	addresses := make([]fnoutil.Address, len(c.Addresses))
	for i := range c.Addresses {
		// Attempt to decode the supplied address.
		addr, err := fnoutil.DecodeAddress(c.Addresses[i])
		if err != nil {
			return nil, rpcAddressKeyError("Could not decode "+
				"address: %v", err)
		}
...
func handleExistsMissedTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.ExistsMissedTicketsCmd)

	hashes, err := fnojson.DecodeConcatenatedHashes(c.TxHashBlob)
	if err != nil {
		return nil, err
	}
...
func handleExistsExpiredTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.ExistsExpiredTicketsCmd)
...
func handleExistsLiveTicket(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.ExistsLiveTicketCmd)
...
func handleExistsLiveTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.ExistsLiveTicketsCmd)
...
func handleExistsMempoolTxs(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.ExistsMempoolTxsCmd)
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.GetAddedNodeInfoCmd)

	// Retrieve a list of persistent (added) peers from the Fonero server
	...
	results := make([]*fnojson.GetAddedNodeInfoResult, 0, len(peers))
	for _, peer := range peers {
		// Set the "address" of the peer which could be an ip address
		// or a domain name.
		var result fnojson.GetAddedNodeInfoResult
		result.AddedNode = peer.Addr()
		result.Connected = fnojson.Bool(peer.Connected())
		...
		addrs := make([]fnojson.GetAddedNodeInfoResultAddr, 0,
			len(ipList))
		for _, ip := range ipList {
			var addr fnojson.GetAddedNodeInfoResultAddr
			addr.Address = ip
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	sbitsFloat := float64(blockHeader.SBits) / fnoutil.AtomsPerCoin
	blockReply := fnojson.GetBlockVerboseResult{
		Hash:          c.Hash,
	...
	if c.VerboseTx == nil || !*c.VerboseTx {
		...
		rawTxns := make([]fnojson.TxRawResult, len(txns))
		for i, tx := range txns {
		...
		rawSTxns := make([]fnojson.TxRawResult, len(stxns))
...
func handleGetBlockchainInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	response := fnojson.GetBlockChainInfoResult{
		Chain:                params.Name,
		Blocks:               best.Height,
		Headers:              best.Height,
...
func handleGetBlockHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.GetBlockHashCmd)
	hash, err := s.chain.BlockHashByHeight(c.Index)
	if err != nil {
		return nil, &fnojson.RPCError{
			Code: fnojson.ErrRPCOutOfRange,
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.GetBlockHeaderCmd)
	...
	blockHeaderReply := fnojson.GetBlockHeaderVerboseResult{
		Hash:          c.Hash,
		Confirmations: confirmations,
		Version:       blockHeader.Version,
		MerkleRoot:    blockHeader.MerkleRoot.String(),
		...
		PoolSize:      blockHeader.PoolSize,
		Bits:          strconv.FormatInt(int64(blockHeader.Bits), 16),
		SBits:         fnoutil.Amount(blockHeader.SBits).ToCoin(),
...
func handleGetBlockSubsidy(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.GetBlockSubsidyCmd)
	...
	rep := fnojson.GetBlockSubsidyResult{
		Developer: dev,
		PoS:       pos,
		PoW:       pow,
		Total:     total,
...
// state as a fnojson.GetBlockTemplateResult that is ready to be encoded to
// JSON and returned to the caller.
//
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(bm *blockManager, useCoinbaseValue bool, submitOld *bool) (*fnojson.GetBlockTemplateResult, error) {
	...
	transactions := make([]fnojson.GetBlockTemplateResultTx, 0, numTx-1)
	...
	for i, tx := range msgBlock.Transactions {
		...
		resultTx := fnojson.GetBlockTemplateResultTx{
	...
	stransactions := make([]fnojson.GetBlockTemplateResultTx, 0, numSTx)
	...
	reply := fnojson.GetBlockTemplateResult{
...
func handleGetBlockTemplateRequest(s *rpcServer, request *fnojson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !cfg.SimNet && s.server.ConnectedCount() == 0 {
		return nil, &fnojson.RPCError{
			...
			Message: "Fonero is not connected",
		}
	}
	...
	if bestHeight != 0 && !s.server.blockManager.IsCurrent() {
		return nil, &fnojson.RPCError{
			Code:    fnojson.ErrRPCClientInInitialDownload,
			Message: "Fonero is downloading blocks...",
		}
	}
...
// reasons.
// TODO Fonero pop in the new errors from blockchain cj
...
func handleGetBlockTemplateProposal(s *rpcServer, request *fnojson.TemplateRequest) (interface{}, error) {
...
func handleGetBlockTemplate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*fnojson.GetBlockTemplateCmd)
	request := c.Request
...
func handleGetChainTips(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	result := make([]fnojson.GetChainTipsResult, 0, len(chainTips))
	for _, tip := range chainTips {
		result = append(result, fnojson.GetChainTipsResult{
			Height:    tip.Height,
...
func handleGetCFilterHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*fnojson.GetCFilterHeaderCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetHeaders(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.GetHeadersCmd)
	blockLocators, err := fnojson.DecodeConcatenatedHashes(c.BlockLocators)
	if err != nil {
		// Already a *fnojson.RPCError
		return nil, err
	}
	...
	return &fnojson.GetHeadersResult{Headers: hexBlockHeaders}, nil
}
...
func handleGetMempoolInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &fnojson.GetMempoolInfoResult{
...
func handleGetMiningInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	gnhpsCmd := fnojson.NewGetNetworkHashPSCmd(nil, nil)
	...
	result := fnojson.GetMiningInfoResult{
		Blocks:           best.Height,
...
func handleGetNetTotals(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	reply := &fnojson.GetNetTotalsResult{
...
func handleGetNetworkHashPS(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*fnojson.GetNetworkHashPSCmd)
...
func handleGetPeerInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	infos := make([]*fnojson.GetPeerInfoResult, 0, len(peers))
	for _, p := range peers {
		statsSnap := p.StatsSnapshot()
		info := &fnojson.GetPeerInfoResult{
			ID:             statsSnap.ID,
			Addr:           statsSnap.Addr,
...
func handleGetRawMempool(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.GetRawMempoolCmd)
	...
	if c.TxType != nil {
		switch fnojson.GetRawMempoolTxTypeCmd(*c.TxType) {
		case fnojson.GRMRegular:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeRegular
		case fnojson.GRMTickets:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeSStx
		case fnojson.GRMVotes:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeSSGen
		case fnojson.GRMRevocations:
			filterType = new(stake.TxType)
...
func handleGetRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.GetRawTransactionCmd)
...
func handleGetStakeDifficulty(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if err != nil {
		...
		return nil, &fnojson.RPCError{
			Code:    fnojson.ErrRPCDifficulty,
			Message: "Error getting stake difficulty: " + err.Error(),
		}
	}
	currentSdiff := fnoutil.Amount(blockHeader.SBits)
	...
	nextSdiffAmount := fnoutil.Amount(nextSdiff)

	sDiffResult := &fnojson.GetStakeDifficultyResult{
...
func convertVersionMap(m map[int]int) []fnojson.VersionCount {
	sorted := make([]fnojson.VersionCount, 0, len(m))
	...
	for _, v := range order {
		sorted = append(sorted, fnojson.VersionCount{Version: uint32(v),
			Count: uint32(m[v])})
	}
...
func handleGetStakeVersionInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	count := int32(1)
	c, ok := cmd.(*fnojson.GetStakeVersionInfoCmd)
	if !ok {
	...
	result := fnojson.GetStakeVersionInfoResult{
		CurrentHeight: snapshot.Height,
		Hash:          snapshot.Hash.String(),
		Intervals:     make([]fnojson.VersionInterval, 0, count),
	}
	...
	for i := int32(0); i < count; i++ {
		...
		versionInterval := fnojson.VersionInterval{
			StartHeight:  endHeight,
...
func handleGetStakeVersions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.GetStakeVersionsCmd)
	...
	result := fnojson.GetStakeVersionsResult{
		StakeVersions: make([]fnojson.StakeVersions, 0, len(sv)),
	}
	for _, v := range sv {
		nsv := fnojson.StakeVersions{
			Hash:         v.Hash.String(),
			Height:       v.Height,
			BlockVersion: v.BlockVersion,
			StakeVersion: v.StakeVersion,
			Votes: make([]fnojson.VersionBits, 0,
				len(v.Votes)),
		}
		for _, vote := range v.Votes {
			nsv.Votes = append(nsv.Votes,
				fnojson.VersionBits{Version: vote.Version,
					Bits: vote.Bits})
		}
...
func handleGetVoteInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c, ok := cmd.(*fnojson.GetVoteInfoCmd)
	if !ok {
		return nil, rpcInvalidError("Invalid type: %T", c)
	}
	...
	// Assemble JSON result.
	result := fnojson.GetVoteInfoResult{
		CurrentHeight: snapshot.Height,
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if txFromMempool != nil {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &fnojson.RPCError{
				Code: fnojson.ErrRPCInvalidTxVout,
	...
	txOutReply := &fnojson.GetTxOutResult{
		BestBlock:     bestBlockHash,
		Confirmations: confirmations,
		Value:         fnoutil.Amount(value).ToUnit(fnoutil.AmountCoin),
		Version:       int32(txVersion),
		ScriptPubKey: fnojson.ScriptPubKeyResult{
			Asm:       disbuf,
...
func handleLiveTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return fnojson.LiveTicketsResult{Tickets: ltString}, nil
}
...
func handleMissedTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return fnojson.MissedTicketsResult{Tickets: mtString}, nil
}
...
// passed transaction.
func createVinListPrevOut(s *rpcServer, mtx *wire.MsgTx, chainParams *chaincfg.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]fnojson.VinPrevOut, error) {
	// Coinbase transactions only have a single txin by definition.
	if blockchain.IsCoinBaseTx(mtx) {
		...
		txIn := mtx.TxIn[0]
		vinList := make([]fnojson.VinPrevOut, 1)
		vinList[0].Coinbase = hex.EncodeToString(txIn.SignatureScript)
		amountIn := fnoutil.Amount(txIn.ValueIn).ToCoin()
	...
	vinList := make([]fnojson.VinPrevOut, 0, len(mtx.TxIn))
	...
	for i, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &fnojson.PrevOut{
				Addresses: encodedAddrs,
				Value:     fnoutil.Amount(originTxOut.Value).ToCoin(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr fnoutil.Address, numToSkip, numRequested uint32) ([]*fnoutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*fnojson.SearchRawTransactionsCmd)
	vinExtra := false
	if c.VinExtra != nil {
	...
	srtList := make([]fnojson.SearchRawTransactionsResult, len(addressTxns))
	for i := range addressTxns {
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.SendRawTransactionCmd)
...
func min(s []fnoutil.Amount) fnoutil.Amount {
	if len(s) == 0 {
		return 0
	}
...
func max(s []fnoutil.Amount) fnoutil.Amount {
	max := fnoutil.Amount(0)
	for i := range s {
		if s[i] > max {
			max = s[i]
		}
...
func mean(s []fnoutil.Amount) fnoutil.Amount {
	sum := fnoutil.Amount(0)
	for i := range s {
		sum += s[i]
	}
	...
	return sum / fnoutil.Amount(len(s))
}

// median gets the median amount from a slice of amounts.
func median(s []fnoutil.Amount) fnoutil.Amount {
	if len(s) == 0 {
		return 0
	}
...
func stdDev(s []fnoutil.Amount) fnoutil.Amount {
	var total float64
	mean := mean(s)
	...
	amt, _ := fnoutil.NewAmount(math.Sqrt(v))
...
// memory pool.
func feeInfoForMempool(s *rpcServer, txType stake.TxType) *fnojson.FeeInfoMempool {
	txDs := s.server.txMemPool.TxDescs()
	ticketFees := make([]fnoutil.Amount, 0, len(txDs))
	for _, txD := range txDs {
		if txD.Type == txType {
			feePerKb := (fnoutil.Amount(txD.Fee)) * 1000 /
				fnoutil.Amount(txD.Tx.MsgTx().SerializeSize())
			ticketFees = append(ticketFees, feePerKb)
		}
	}

	return &fnojson.FeeInfoMempool{
...
// properly set.
func calcFeePerKb(tx *fnoutil.Tx) fnoutil.Amount {
	var in fnoutil.Amount
	for _, txIn := range tx.MsgTx().TxIn {
		in += fnoutil.Amount(txIn.ValueIn)
	}
	var out fnoutil.Amount
	for _, txOut := range tx.MsgTx().TxOut {
		out += fnoutil.Amount(txOut.Value)
	}

	return ((in - out) * 1000) / fnoutil.Amount(tx.MsgTx().SerializeSize())
}

// feeInfoForBlock fetches the ticket fee information for a given tx type in a
// block.
func ticketFeeInfoForBlock(s *rpcServer, height int64, txType stake.TxType) (*fnojson.FeeInfoBlock, error) {
	...
	txFees := make([]fnoutil.Amount, txNum)
	itr := 0
...
// from [start, end).
func ticketFeeInfoForRange(s *rpcServer, start int64, end int64, txType stake.TxType) (*fnojson.FeeInfoWindow, error) {
	hashes, err := s.chain.HeightRange(start, end)
	if err != nil {
		return nil, err
	}

	var txFees []fnoutil.Amount
	for i := range hashes {
...
func handleTicketFeeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.TicketFeeInfoCmd)
	...
	var feeInfoBlocks []fnojson.FeeInfoBlock
	blocks := uint32(0)
	if c.Blocks != nil {
		blocks = *c.Blocks
	}
	...
	var feeInfoWindows []fnojson.FeeInfoWindow
	windows := uint32(0)
	if c.Windows != nil {
		windows = *c.Windows
	}
	...
	return &fnojson.TicketFeeInfoResult{
...
func handleTicketsForAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.TicketsForAddressCmd)
	...
	reply := &fnojson.TicketsForAddressResult{
...
func handleTicketVWAP(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return fnoutil.Amount(vwap).ToCoin(), nil
}
...
func handleTxFeeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	var feeInfoRange fnojson.FeeInfoRange
	...
	feeInfoRange = fnojson.FeeInfoRange{
	...
	return &fnojson.TxFeeInfoResult{
		FeeInfoMempool: *feeInfoMempool,
		FeeInfoBlocks:  feeInfoBlocks,
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.ValidateAddressCmd)
	result := fnojson.ValidateAddressChainResult{}
	addr, err := fnoutil.DecodeAddress(c.Address)
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*fnojson.VerifyMessageCmd)
	...
	address, err := fnoutil.NewAddressSecpPubKey(serializedPK,
		activeNetParams.Params)
	if err != nil {
...
func handleVersion(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	result := map[string]fnojson.VersionResult{
		"fnodjsonrpcapi": {
...
func (s *rpcServer) standardCmdResult(cmd *parsedRPCCmd, closeChan <-chan struct{}) (interface{}, error) {
	...
	return nil, fnojson.ErrRPCMethodNotFound
handled:
...
func parseCmd(request *fnojson.Request) *parsedRPCCmd {
	...
	if err != nil {
		...
		if jerr, ok := err.(fnojson.Error); ok &&
			jerr.Code == fnojson.ErrUnregisteredMethod {
			parsedCmd.err = fnojson.ErrRPCMethodNotFound
			return &parsedCmd
		}
...
// type *fnojson.RPCError to the appropriate type as needed.
func createMarshalledReply(rpcVersion string, id interface{}, result interface{}, replyErr error) ([]byte, error) {
	...
	if replyErr != nil {
		if jErr, ok := replyErr.(*fnojson.RPCError); ok {
			jsonErr = jErr
		} else {
	...
	return fnojson.MarshalResponse(rpcVersion, id, result, jsonErr)
}

// processRequest determines the incoming request type (single or batched),
// parses it and returns a marshalled response.
func (s *rpcServer) processRequest(request *fnojson.Request, isAdmin bool, closeChan <-chan struct{}) []byte {
...
func (s *rpcServer) jsonRPCRead(w http.ResponseWriter, r *http.Request, isAdmin bool) {
	...
	if !batchedRequest {
		...
		if err != nil {
			...
			resp, err = fnojson.MarshalResponse("1.0", nil, nil, jsonErr)
			if err != nil {
	...
	if batchedRequest {
		...
		if err != nil {
			...
			resp, err = fnojson.MarshalResponse("2.0", nil, nil, jsonErr)
			if err != nil {
				rpcsLog.Errorf("Failed to create reply: %v", err)
			}
			...
			if len(batchedRequests) > 0 {
				...
				for _, entry := range batchedRequests {
					...
					if err != nil {
						...
						resp, err = fnojson.MarshalResponse("", nil, nil, jsonErr)
						if err != nil {
							rpcsLog.Errorf("Failed to create reply: %v", err)
						}
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="fnod RPC"`)
