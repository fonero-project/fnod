...
func containsTxIns(txs []*fnoutil.Tx, tx *fnoutil.Tx) bool {
...
// can not be found.
func txIndexFromTxList(hash chainhash.Hash, list []*fnoutil.Tx) int {
	for i, tx := range list {
		h := tx.Hash()
		if hash == *h {
			return i
...
func UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int64, extraNonce uint64) error {
	...
	// TODO(davec): A fnoutil.Block should use saved in the state to avoid
	// recalculating all of the other transaction hashes.
	// block.Transactions[0].InvalidateCache()

	// Recalculate the merkle root with the updated extra nonce.
	block := fnoutil.NewBlockDeepCopyCoinbase(msgBlock)
...
func createCoinbaseTx(subsidyCache *blockchain.SubsidyCache, coinbaseScript []byte, opReturnPkScript []byte, nextBlockHeight int64, addr fnoutil.Address, voters uint16, params *chaincfg.Params) (*fnoutil.Tx, error) {
	tx := wire.NewMsgTx()
	...
	if nextBlockHeight == 1 && len(params.BlockOneLedger) != 0 {
		...
		addrs := make([]fnoutil.Address, len(params.BlockOneLedger))
		for i, payout := range params.BlockOneLedger {
			addr, err := fnoutil.DecodeAddress(payout.Address)
			if err != nil {
				return nil, err
			}
			addrs[i] = addr
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *fnoutil.Tx, height int64) error {
	for _, txIn := range tx.MsgTx().TxIn {
...
func logSkippedDeps(tx *fnoutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
func maybeInsertStakeTx(bm *blockManager, stx *fnoutil.Tx, treeValid bool) bool {
	missingInput := false
...
func deepCopyBlockTemplate(blockTemplate *BlockTemplate) *BlockTemplate {
	...
	coinbaseCopy :=
		fnoutil.NewTxDeep(blockTemplate.Block.Transactions[0])
...
func handleTooFewVoters(subsidyCache *blockchain.SubsidyCache, nextHeight int64, miningAddress fnoutil.Address, bm *blockManager) (*BlockTemplate, error) {
	...
	if nextHeight >= stakeValidationHeight {
		if bm.AggressiveMining {
			if curTemplate != nil {
				...
				block := fnoutil.NewBlockDeepCopyCoinbase(cptCopy.Block)
			...
			if curTemplate == nil {
				...
				btBlockTemp := fnoutil.NewBlockDeepCopyCoinbase(btMsgBlock)
				merkles :=
				...
				// Make sure the block validates.
				btBlock := fnoutil.NewBlockDeepCopyCoinbase(btMsgBlock)
...
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress fnoutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*fnoutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		// Consider caching this in the mempool in the future. - Fonero
	...
	blockTxnsStake := make([]*fnoutil.Tx, 0, len(blockTxns))
	...
	for _, tx := range blockTxns {
		...
		if stake.IsSSGen(msgTx) {
			txCopy := fnoutil.NewTxDeepTxIns(msgTx)
	...
	if nextBlockHeight < stakeValidationHeight {
		...
		for _, vb := range voteBitsVoters {
			if fnoutil.IsFlagSet16(vb, fnoutil.BlockValid) {
				voteYea++
			}
			totalVotes++
		}
		...
		if votebits == uint16(0x0000) {
			...
			tempBlockTxns := make([]*fnoutil.Tx, 0, len(sourceTxns))
			for _, tx := range blockTxns {
				if tx.Tree() == wire.TxTreeRegular {
					...
					if isValid {
						txCopy := fnoutil.NewTxDeepTxIns(tx.MsgTx())
	...
	// Fonero: We need to move this downwards because of the requirements
	...
	blockTxnsRegular := make([]*fnoutil.Tx, 0, len(blockTxns)+1)
	...
	minrLog.Debugf("Created new block template (%d transactions, %d "+
		...
		fnoutil.Amount(msgBlock.Header.SBits).ToCoin())
