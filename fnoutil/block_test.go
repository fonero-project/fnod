...
func TestBlock(t *testing.T) {
	b := fnoutil.NewBlock(&Block100000)
	...
	wantHashStr := "6b5e8a99e97c113f1273a62f29a30108d3cec4f4eae40db37dd250682ac74d76"
...
func TestNewBlockFromBytes(t *testing.T) {
	...
	b, err := fnoutil.NewBlockFromBytes(block100000Bytes)
	if err != nil {
...
func TestNewBlockFromBlockAndBytes(t *testing.T) {
	...
	// Create a new block from the serialized bytes.
	b := fnoutil.NewBlockFromBlockAndBytes(&Block100000, block100000Bytes)
...
func TestBlockErrors(t *testing.T) {
	...
	testErr := fnoutil.OutOfRangeError(wantErr)
	...
	_, err = fnoutil.NewBlockFromBytes(shortBytes)
	if err != io.EOF {
	...
	_, err = b.TxHash(-1)
	if _, ok := err.(fnoutil.OutOfRangeError); !ok {
		t.Errorf("TxHash: wrong error - got: %v <%T>, "+
			"want: <%T>", err, err, fnoutil.OutOfRangeError(""))
	}
