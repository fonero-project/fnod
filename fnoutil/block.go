...
// Tx returns a wrapped transaction (fnoutil.Tx) for the transaction at the
...
// STx returns a wrapped transaction (fnoutil.Tx) for the stake transaction at
...
// Transactions returns a slice of wrapped transactions (fnoutil.Tx) for all
...
// instead provides easy access to wrapped versions (fnoutil.Tx) of them.
...
// STransactions returns a slice of wrapped stake transactions (fnoutil.Tx) for all
// stake transactions in the Block.  This is nearly equivalent to accessing the raw
// transactions (fnowire.MsgTx) in the underlying wire.MsgBlock, however it
