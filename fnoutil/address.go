...
import (
	...
	"github.com/fonero-project/fnod/fnoec/secp256k1/schnorr"
)
...
func encodePKAddress(serializedPK []byte, netID [2]byte, algo fnoec.SignatureType) string {
	pubKeyBytes := []byte{0x00}

	switch algo {
	case fnoec.STEcdsaSecp256k1:
		pubKeyBytes[0] = byte(fnoec.STEcdsaSecp256k1)
	case fnoec.STEd25519:
		pubKeyBytes[0] = byte(fnoec.STEd25519)
	case fnoec.STSchnorrSecp256k1:
		pubKeyBytes[0] = byte(fnoec.STSchnorrSecp256k1)
	}

	// Pubkeys are encoded as [0] = type/ybit, [1:33] = serialized pubkey
	compressed := serializedPK
	if algo == fnoec.STEcdsaSecp256k1 || algo == fnoec.STSchnorrSecp256k1 {
...
type Address interface {
	...
	DSA(*chaincfg.Params) fnoec.SignatureType
...
func NewAddressPubKey(decoded []byte, net *chaincfg.Params) (Address, error) {
	if len(decoded) == 33 {
		...
		switch fnoec.SignatureType(suite) {
		case fnoec.STEcdsaSecp256k1:
...
func DecodeAddress(addr string) (Address, error) {
	...
	case net.PubKeyHashAddrID:
		return NewAddressPubKeyHash(decoded, net, fnoec.STEcdsaSecp256k1)

	case net.PKHEdwardsAddrID:
		return NewAddressPubKeyHash(decoded, net, fnoec.STEd25519)

	case net.PKHSchnorrAddrID:
		return NewAddressPubKeyHash(decoded, net, fnoec.STSchnorrSecp256k1)
...
// be 20 bytes.
func NewAddressPubKeyHash(pkHash []byte, net *chaincfg.Params, algo fnoec.SignatureType) (*AddressPubKeyHash, error) {
	var addrID [2]byte
	switch algo {
	case fnoec.STEcdsaSecp256k1:
...
// hash.
func (a *AddressPubKeyHash) DSA(net *chaincfg.Params) fnoec.SignatureType {
	switch a.netID {
	case net.PubKeyHashAddrID:
		return fnoec.STEcdsaSecp256k1
	case net.PKHEdwardsAddrID:
		return fnoec.STEd25519
	case net.PKHSchnorrAddrID:
		return fnoec.STSchnorrSecp256k1
	}
	return -1
}
...
func (a *AddressScriptHash) DSA(net *chaincfg.Params) fnoec.SignatureType {
	return -1
}
...
func NewAddressSecpPubKey(serializedPubKey []byte,
	...
	// from fnoec, but do it here to avoid API churn.  We already know the
...
// differs with the format.  At the time of this writing, most Fonero addresses
...
// address.
func (a *AddressSecpPubKey) DSA(net *chaincfg.Params) fnoec.SignatureType {
...
// DSA returns the underlying digital signature algorithm for the
// address.
func (a *AddressEdwardsPubKey) DSA(net *chaincfg.Params) fnoec.SignatureType {
	return fnoec.STEd25519
}
...
// DSA returns the underlying digital signature algorithm for the
// address.
func (a *AddressSecSchnorrPubKey) DSA(net *chaincfg.Params) fnoec.SignatureType {
	return fnoec.STSchnorrSecp256k1
}
