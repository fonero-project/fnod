...
A Block defines a Fonero block that provides easier and more efficient
...
A Tx defines a Fonero transaction that provides more efficient manipulation of
...
The Address interface provides an abstraction for a Fonero address.  While the
most common type is a pay-to-pubkey-hash, Fonero already supports others and
...
To decode/encode an address:
	...
	addr, err := fnoutil.DecodeAddress(addrString, defaultNet)
	if err != nil {
		fmt.Println(err)
		return
	}
