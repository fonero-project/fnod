...
// OutPoint defines a Fonero data type that is used to track previous
// transaction outputs.
...
// NewOutPoint returns a new Fonero transaction outpoint point with the
...
// NewTxIn returns a new Fonero transaction input with the provided
...
// NewTxOut returns a new Fonero transaction output with the provided
...
// MsgTx implements the Message interface and represents a Fonero tx message.
...
func writeTxScriptsToMsgTx(msg *MsgTx, totalScriptSize uint64, serType TxSerializeType) {
	...
	//
	// Closures around writing the TxIn and TxOut scripts are used in Fonero
...
// in that BtcDecode decodes from the Fonero wire protocol as it was sent
...
// encodes the transaction to the Fonero wire protocol in order to be sent
// across the network.  The wire encoding can technically differ depending on
// the protocol version and doesn't even really need to match the format of a
// stored transaction at all.  As of the time this comment was written, the
// encoded transaction is the same in both instances, but there is a distinct
...
// NewMsgTx returns a new Fonero tx message that conforms to the Message
...
// WriteOutPoint encodes op to the Fonero protocol encoding for an OutPoint
// to w.
...
// writeTxInPrefixs encodes ti to the Fonero protocol encoding for a transaction
...
// writeTxWitness encodes ti to the Fonero protocol encoding for a transaction
...
// writeTxOut encodes to into the Fonero protocol encoding for a transaction
// output (TxOut) to w.
