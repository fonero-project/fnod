...
func (stats *Estimator) EstimateFee(targetConfs int32) (fnoutil.Amount, error) {
	stats.lock.RLock()
	...
	return fnoutil.Amount(rate), nil
}
...
func (stats *Estimator) AddMemPoolTransaction(txHash *chainhash.Hash, fee, size int64, txType stake.TxType) {
	...
	// minimum at values less than 0.001 FNO/KB. This is needed because due to
...
func (stats *Estimator) ProcessBlock(block *fnoutil.Block) error {
	stats.lock.Lock()
	defer stats.lock.Unlock()
