...
import (
	...
	"github.com/fonero-project/fnod/fees"
	...
	"github.com/fonero-project/fnod/peer"
...
var (
	...
	// ourselves to other Fonero peers.
	userAgentName = "fnod"
...
// broadcastMsg provides the ability to house a Fonero message to be broadcast
...
// server provides a Fonero server for handling communications to and from
// Fonero peers.
type server struct {
...
func (sp *serverPeer) OnTx(p *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a fnoutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := fnoutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(p *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a fnoutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := fnoutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (s *server) AnnounceNewTransactions(newTxs []*fnoutil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices, fnodLookup, func(addrs []*wire.NetAddress) {
...
// AddedNodeInfo returns an array of fnojson.GetAddedNodeInfoResult structures
...
// newServer returns a new fnod server configured to listen on addr for the
// Fonero network type specified by chainParams.  Use start to begin accepting
...
func newServer(listenAddrs []string, db database.DB, chainParams *chaincfg.Params, dataDir string, interrupt <-chan struct{}) (*server, error) {
	...
	amgr := addrmgr.New(cfg.DataDir, fnodLookup)
	...
	feC := fees.EstimatorConfig{
		...
		MaxBucketFee: fnoutil.Amount(fees.DefaultMaxBucketFeeMultiplier) * cfg.minRelayTxFee,
...
func addrStringToNetAddr(addr string) (net.Addr, error) {
	...
	// The fnodLookup function will transparently handle performing the
	// lookup over Tor if necessary.
	ips, err := fnodLookup(host)
	if err != nil {
		return nil, err
	}
	if len(ips) == 0 {
