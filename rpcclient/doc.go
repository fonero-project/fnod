...
/*
Package rpcclient implements a websocket-enabled Fonero JSON-RPC client.

Overview

This client provides a robust and easy to use client for interfacing
with a Fonero RPC server that uses a mostly btcd/bitcoin core
style Fonero JSON-RPC API.  This client has been tested with fnod
(https://github.com/fonero-project/fnod) and fnowallet
(https://github.com/fonero-project/fnowallet).

In addition to the compatible standard HTTP POST JSON-RPC API, fnod and
fnowallet provide a websocket interface that is more efficient than the standard
...
fnod or fnowallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by fnod and
fnowallet only uses a single connection that remains open and allows
...
All notifications provided by fnod require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by fnod (and
fnowallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that fnod intentionally separates the wallet
functionality into a separate process named fnowallet.  This means if you are
connected to the fnod RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, fnowallet provides pass through treatment
...
detected by type asserting the error in a *fnojson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

  amount, err := client.GetBalance("")
  if err != nil {
  	if jerr, ok := err.(*fnojson.RPCError); ok {
  		switch jerr.Code {
  		case fnojson.ErrRPCUnimplemented:
...
The following full-blown client examples are in the examples directory:
 ...
 - fnodwebsockets
   ...
   block count
 - fnowalletwebsockets
   Connects to a fnowallet RPC server using TLS-secured websockets, registers
