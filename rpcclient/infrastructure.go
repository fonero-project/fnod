...
// Client represents a Fonero RPC client which allows easy access to the
// various RPC methods available on a Fonero RPC server.  Each of the wrapper
...
func (c *Client) trackRegisteredNtfns(cmd interface{}) {
	...
	case *fnojson.NotifyWinningTicketsCmd:
		c.ntfnState.notifyWinningTickets = true

	case *fnojson.NotifySpentAndMissedTicketsCmd:
		c.ntfnState.notifySpentAndMissedTickets = true

	case *fnojson.NotifyNewTicketsCmd:
		c.ntfnState.notifyNewTickets = true

	case *fnojson.NotifyStakeDifficultyCmd:
		c.ntfnState.notifyStakeDifficulty = true

	case *fnojson.NotifyBlocksCmd:
		c.ntfnState.notifyBlocks = true

	case *fnojson.NotifyNewTransactionsCmd:
...
type (
	...
	rawResponse struct {
		...
		Error  *fnojson.RPCError `json:"error"`
	}
)
...
// returning an unmarshaled fnojson.RPCError (or an unmarshaling error) if so.
...
func (c *Client) sendCmd(cmd interface{}) chan *response {
	...
	method, err := fnojson.CmdMethod(cmd)
	if err != nil {
	...
	// Marshal the command.
	id := c.NextID()
	marshalledJSON, err := fnojson.MarshalCmd("1.0", id, cmd)
	if err != nil {
		return newFutureError(err)
	}
