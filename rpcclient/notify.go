...
type NotificationHandlers struct {
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount fnoutil.Amount)
	...
	// such as fnowallet.
	OnAccountBalance func(account string, balance fnoutil.Amount, confirmed bool)
	...
	// This will only be available when client is connected to a wallet
	// server such as fnowallet.
	OnTicketsPurchased func(TxHash *chainhash.Hash, amount fnoutil.Amount)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	switch ntfn.Method {
	// OnBlockConnected
	case fnojson.BlockConnectedNtfnMethod:
	...
	// OnBlockDisconnected
	case fnojson.BlockDisconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case fnojson.RelevantTxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case fnojson.ReorganizationNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnWinningTickets
	case fnojson.WinningTicketsNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case fnojson.SpentAndMissedTicketsNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnNewTickets
	case fnojson.NewTicketsNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnStakeDifficulty
	case fnojson.StakeDifficultyNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAccepted
	case fnojson.TxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAcceptedVerbose
	case fnojson.TxAcceptedVerboseNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
		...
		// OnDcrdConnected
	case fnojson.DcrdConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
		...
		if err != nil {
			log.Warnf("Received invalid fnod connected "+
				"notification: %v", err)
			return
		}
	...
	// OnAccountBalance
	case fnojson.AccountBalanceNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTicketPurchased:
	case fnojson.TicketPurchasedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnVotesCreated:
	case fnojson.VoteCreatedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case fnojson.RevocationCreatedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnWalletLockState
	case fnojson.WalletLockStateNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := fnoutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
func parseTxAcceptedVerboseNtfnParams(params []json.RawMessage) (*fnojson.TxRawResult,
	error) {
...
// parseDcrdConnectedNtfnParams parses out the connection status of fnod
// and fnowallet from the parameters of a fnodconnected notification.
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance fnoutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := fnoutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
func parseTicketPurchasedNtfnParams(params []json.RawMessage) (txHash *chainhash.Hash,
	amount fnoutil.Amount, err error) {
	...
	return thHash, fnoutil.Amount(amt), nil
}
...
//
// NOTE: This is a fnod extension and requires a websocket connection.
func (c *Client) NotifyBlocksAsync() FutureNotifyBlocksResult {
	...
	cmd := fnojson.NewNotifyBlocksCmd()
...
func (c *Client) NotifyWinningTicketsAsync() FutureNotifyWinningTicketsResult {
	...
	cmd := fnojson.NewNotifyWinningTicketsCmd()
...
func (c *Client) NotifySpentAndMissedTicketsAsync() FutureNotifySpentAndMissedTicketsResult {
	...
	cmd := fnojson.NewNotifySpentAndMissedTicketsCmd()
...
func (c *Client) NotifyNewTicketsAsync() FutureNotifyNewTicketsResult {
	...
	cmd := fnojson.NewNotifyNewTicketsCmd()
...
func (c *Client) NotifyStakeDifficultyAsync() FutureNotifyStakeDifficultyResult {
	...
	cmd := fnojson.NewNotifyStakeDifficultyCmd()
...
func (c *Client) NotifyNewTransactionsAsync(verbose bool) FutureNotifyNewTransactionsResult {
	...
	cmd := fnojson.NewNotifyNewTransactionsCmd(&verbose)
	return c.sendCmd(cmd)
}
...
//
// NOTE: This is a fnod extension and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []fnoutil.Address,
	...
	outPointObjects := make([]fnojson.OutPoint, len(outPoints))
	for i := range outPoints {
		outPointObjects[i] = fnojson.OutPoint{
	...
	cmd := fnojson.NewLoadTxFilterCmd(reload, addrStrs, outPointObjects)
	return c.sendCmd(cmd)
}
...
//
// NOTE: This is a fnod extension and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []fnoutil.Address, outPoints []wire.OutPoint) error {
