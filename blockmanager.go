...
// blockMsg packages a Fonero block message and the peer it came from together
...
// invMsg packages a Fonero inv message and the peer it came from together
// so the block handler has access to that information.
type invMsg struct {
	inv  *wire.MsgInv
	peer *serverPeer
...
// headersMsg packages a Fonero headers message and the peer it came from
...
// txMsg packages a Fonero tx message and the peer it came from together
// so the block handler has access to that information.
type txMsg struct {
	tx   *fnoutil.Tx
	peer *serverPeer
...
func (b *blockManager) checkBlockForHiddenVotes(block *fnoutil.Block) {
	var votesFromBlock []*fnoutil.Tx
	...
	var newVotes []*fnoutil.Tx
	var oldTickets []*fnoutil.Tx
	var oldRevocations []*fnoutil.Tx
	oldVoteMap := make(map[chainhash.Hash]struct{},
		int(b.server.chainParams.TicketsPerBlock))
	if template != nil {
		templateBlock := fnoutil.NewBlock(template.Block)
	...
	updatedTxTreeStake := make([]*fnoutil.Tx, 0,
	...
	var updatedTxTreeRegular []*fnoutil.Tx
...
func headerApprovesParent(header *wire.BlockHeader) bool {
	return fnoutil.IsFlagSet16(header.VoteBits, fnoutil.BlockValid)
}
...
func (b *blockManager) handleNotifyMsg(notification *blockchain.Notification) {
	...
	case blockchain.NTNewTipBlockChecked:
		...
		block, ok := notification.Data.(*fnoutil.Block)
		if !ok {
	...
	case blockchain.NTBlockConnected:
		blockSlice, ok := notification.Data.([]*fnoutil.Block)
		if !ok {
		...
		handleConnectedBlockTxns := func(txns []*fnoutil.Tx) {
			for _, tx := range txns {
	...
	case blockchain.NTBlockDisconnected:
		...
		handleDisconnectedBlockTxns := func(txns []*fnoutil.Tx) {
			for _, tx := range txns {
				_, err := txMemPool.MaybeAcceptTransaction(tx, false, true)
				if err != nil && !isDoubleSpendOrDuplicateError(err) {
					txMemPool.RemoveTransaction(tx, true)
...
// queue.
func (b *blockManager) QueueTx(tx *fnoutil.Tx, sp *serverPeer) {
...
func (b *blockManager) QueueBlock(block *fnoutil.Block, sp *serverPeer) {
...
func (b *blockManager) ProcessBlock(block *fnoutil.Block, flags blockchain.BehaviorFlags) (bool, error) {
...
func (b *blockManager) ProcessTransaction(tx *fnoutil.Tx, allowOrphans bool,
	rateLimit bool, allowHighFees bool) ([]*fnoutil.Tx, error) {
...
// pool.
func (b *blockManager) TicketPoolValue() (fnoutil.Amount, error) {
...
// newBlockManager returns a new Fonero block manager.
...
func removeRegressionDB(dbPath string) error {
	...
	if err == nil {
		fnodLog.Infof("Removing regression test database from '%s'", dbPath)
		if fi.IsDir() {
...
func warnMultipleDBs() {
	...
	if len(duplicateDbPaths) > 0 {
		...
		fnodLog.Warnf("WARNING: There are multiple block chain databases "+
...
func loadBlockDB() (database.DB, error) {
	...
	if cfg.DbType == "memdb" {
		fnodLog.Infof("Creating block database in memory.")
	...
	fnodLog.Infof("Loading block database from '%s'", dbPath)
	...
	fnodLog.Info("Block database loaded")
	return db, nil
}
