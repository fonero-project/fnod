...
import (
	...
	_ "github.com/fonero-project/fnod/database/ffldb"
	"github.com/fonero-project/fnod/internal/version"
	"github.com/fonero-project/fnod/mempool"
	"github.com/fonero-project/fnod/sampleconfig"
	"github.com/fonero-project/fnod/fnoutil"
)

const (
	defaultConfigFilename        = "fnod.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "fnod.log"
	defaultMaxSameIP             = 5
	defaultMaxPeers              = 125
...
var (
	defaultHomeDir     = fnoutil.AppDataDir("fnod", false)
...
type config struct {
	...
	MinRelayTxFee        float64       `long:"minrelaytxfee" description:"The minimum transaction fee in FNO/kB to be considered a non-zero fee."`
	...
	AltDNSNames          []string      `long:"altdnsnames" description:"Specify additional dns names to use when generating the rpc server certificate" env:"FNOD_ALT_DNSNAMES" env-delim:","`
	onionlookup          func(string) ([]net.IP, error)
	lookup               func(string) ([]net.IP, error)
	oniondial            func(string, string) (net.Conn, error)
	dial                 func(string, string) (net.Conn, error)
	miningAddrs          []fnoutil.Address
...
// createDefaultConfig copies the file sample-fnod.conf to the given destination path,
...
//
// The above results in fnod functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	// Update the home directory for fnod if specified. Since the home
	...
	cfg.minRelayTxFee, err = fnoutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]fnoutil.Address, 0, len(cfg.GetWorkKeys)+
		len(cfg.MiningAddrs))
	for _, strAddr := range cfg.GetWorkKeys {
		addr, err := fnoutil.DecodeAddress(strAddr)
		if err != nil {
	...
	for _, oldDir := range oldTestNets {
		if fileExists(oldDir) {
			fnodLog.Warnf("Block chain data from previous testnet"+
	...
	if configFileError != nil {
		fnodLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// fnodDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func fnodDial(network, addr string) (net.Conn, error) {
...
// fnodLookup returns the correct DNS lookup function to use depending on the
...
func fnodLookup(host string) ([]net.IP, error) {
