...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *fnoutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *fnoutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *fnoutil.Tx, isNew bool) {
...
func (f *wsClientFilter) addAddress(a fnoutil.Address) {
	switch a := a.(type) {
...
func (f *wsClientFilter) existsAddress(a fnoutil.Address) bool {
	switch a := a.(type) {
	case *fnoutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected fnoutil.Block
type notificationBlockDisconnected fnoutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *fnoutil.Tx, clients map[chan struct{}]*wsClient) map[chan struct{}]struct{} {
...
func (m *wsNotificationManager) notifyBlockConnected(clients map[chan struct{}]*wsClient, block *fnoutil.Block) {
	...
	ntfn := fnojson.BlockConnectedNtfn{
	...
	for quitChan, client := range clients {
		...
		marshalledJSON, err := fnojson.MarshalCmd("1.0", nil, &ntfn)
		if err != nil {
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *fnoutil.Block) {
	...
	ntfn := fnojson.BlockDisconnectedNtfn{
		Header: hex.EncodeToString(headerBytes),
	}
	marshalledJSON, err := fnojson.MarshalCmd("1.0", nil, &ntfn)
	if err != nil {
...
func (m *wsNotificationManager) notifyReorganization(clients map[chan struct{}]*wsClient, rd *blockchain.ReorganizationNtfnsData) {
	...
	// Notify interested websocket clients about the disconnected block.
	ntfn := fnojson.NewReorganizationNtfn(rd.OldHash.String(),
		int32(rd.OldHeight),
		rd.NewHash.String(),
		int32(rd.NewHeight))
	marshalledJSON, err := fnojson.MarshalCmd("1.0", nil, ntfn)
...
func (*wsNotificationManager) notifyWinningTickets(
	...
	ntfn := fnojson.NewWinningTicketsNtfn(wtnd.BlockHash.String(),
...
func (*wsNotificationManager) notifySpentAndMissedTickets(clients map[chan struct{}]*wsClient, tnd *blockchain.TicketNotificationsData) {
	...
	// Notify interested websocket clients about the connected block.
	ntfn := fnojson.NewSpentAndMissedTicketsNtfn(tnd.Hash.String(),
...
func (*wsNotificationManager) notifyNewTickets(clients map[chan struct{}]*wsClient, tnd *blockchain.TicketNotificationsData) {
	...
	// Notify interested websocket clients about the connected block.
	ntfn := fnojson.NewNewTicketsNtfn(tnd.Hash.String(), int32(tnd.Height),
...
func (*wsNotificationManager) notifyStakeDifficulty(clients map[chan struct{}]*wsClient, sdnd *StakeDifficultyNtfnData) {
	// Notify interested websocket clients about the connected block.
	ntfn := fnojson.NewStakeDifficultyNtfn(sdnd.BlockHash.String(),
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *fnoutil.Tx) {
	...
	ntfn := fnojson.NewTxAcceptedNtfn(txHashStr,
		fnoutil.Amount(amount).ToCoin())
	marshalledJSON, err := fnojson.MarshalCmd("1.0", nil, ntfn)
	if err != nil {
	...
	var verboseNtfn *fnojson.TxAcceptedVerboseNtfn
	...
	for _, wsc := range clients {
		if wsc.verboseTxUpdates {
			...
			verboseNtfn = fnojson.NewTxAcceptedVerboseNtfn(*rawTx)
			marshalledJSONVerbose, err = fnojson.MarshalCmd("1.0", nil,
				verboseNtfn)
			if err != nil {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *fnoutil.Tx,
	...
	if len(clientsToNotify) != 0 {
		n := fnojson.NewRelevantTxAcceptedNtfn(txHexString(msgTx))
		marshalled, err := fnojson.MarshalCmd("1.0", nil, n)
		if err != nil {
	...
	for {
		...
		if !batchedRequest {
			...
			if err != nil {
				...
				jsonErr := &fnojson.RPCError{
					Code:    fnojson.ErrRPCParse.Code,
			...
			if req.Method == "" || req.Params == nil {
				jsonErr := &fnojson.RPCError{
					Code:    fnojson.ErrRPCInvalidRequest.Code,
			...
			switch authCmd, ok := cmd.cmd.(*fnojson.AuthenticateCmd); {
			...
			if !c.isAdmin {
				if _, ok := rpcLimited[req.Method]; !ok {
					jsonErr := &fnojson.RPCError{
						Code:    fnojson.ErrRPCInvalidParams.Code,
		...
		if batchedRequest {
			...
			if err != nil {
				...
				reply, err = fnojson.MarshalResponse("2.0", nil, nil, jsonErr)
				if err != nil {
...
func handleWebsocketHelp(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*fnojson.HelpCmd)
	if !ok {
		return nil, fnojson.ErrRPCInternal
	}
...
func handleLoadTxFilter(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.LoadTxFilterCmd)
...
func handleSession(wsc *wsClient, icmd interface{}) (interface{}, error) {
	return &fnojson.SessionResult{SessionID: wsc.sessionID}, nil
}
...
func handleNotifyNewTransactions(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*fnojson.NotifyNewTransactionsCmd)
	if !ok {
		return nil, fnojson.ErrRPCInternal
	}
...
// string slice.
func rescanBlock(filter *wsClientFilter, block *fnoutil.Block) []string {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	...
	if filter == nil {
		return nil, &fnojson.RPCError{
			Code:    fnojson.ErrRPCMisc,
			Message: "Transaction filter must be loaded before rescanning",
		}
	}

	blockHashes, err := fnojson.DecodeConcatenatedHashes(cmd.BlockHashes)
	if err != nil {
		return nil, err
	}

	discoveredData := make([]fnojson.RescannedBlock, 0, len(blockHashes))
	...
	for i := range blockHashes {
		...
		if err != nil {
			return nil, &fnojson.RPCError{
				Code:    fnojson.ErrRPCBlockNotFound,
		...
		if len(transactions) != 0 {
			discoveredData = append(discoveredData, fnojson.RescannedBlock{
	...
	return &fnojson.RescanResult{DiscoveredData: discoveredData}, nil
}
