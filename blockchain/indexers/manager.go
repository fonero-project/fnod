...
import (
	...
	"github.com/fonero-project/fnod/blockchain/internal/progresslog"
...
func dbIndexConnectBlock(dbTx database.Tx, indexer Indexer, block, parent *fnoutil.Block, view *blockchain.UtxoViewpoint) error {
...
func dbIndexDisconnectBlock(dbTx database.Tx, indexer Indexer, block, parent *fnoutil.Block, view *blockchain.UtxoViewpoint) error {
...
// block for the provided hash, deserialize it, and return a fnoutil.Block.
func dbFetchBlockByHash(dbTx database.Tx, hash *chainhash.Hash) (*fnoutil.Block, error) {
	blockBytes, err := dbTx.FetchBlock(hash)
	if err != nil {
		return nil, err
	}
	return fnoutil.NewBlockFromBytes(blockBytes)
}
...
func (m *Manager) Init(chain *blockchain.BlockChain, interrupt <-chan struct{}) error {
	...
	var cachedParent *fnoutil.Block
	for height := lowestHeight + 1; height <= bestHeight; height++ {
		if interruptRequested(interrupt) {
			return errInterruptRequested
		}

		var block, parent *fnoutil.Block
		err = m.db.Update(func(dbTx database.Tx) error {
...
func makeUtxoView(dbTx database.Tx, block *fnoutil.Block, interrupt <-chan struct{}) (*blockchain.UtxoViewpoint, error) {
	view := blockchain.NewUtxoViewpoint()
	processTxns := func(txns []*fnoutil.Tx, regularTree bool) error {
		for txIdx, tx := range txns {
			...
			for txInIdx, txIn := range msgTx.TxIn {
				...
				view.AddTxOuts(fnoutil.NewTx(originTx), int64(txIn.BlockHeight),
					txIn.BlockIndex)
			}
...
//
// This is part of the blockchain.IndexManager interface.
func (m *Manager) ConnectBlock(dbTx database.Tx, block, parent *fnoutil.Block, view *blockchain.UtxoViewpoint) error {
...
//
// This is part of the blockchain.IndexManager interface.
func (m *Manager) DisconnectBlock(dbTx database.Tx, block, parent *fnoutil.Block, view *blockchain.UtxoViewpoint) error {
	// Call each of the currently active optional indexes with the block
