...
func dbAddTxIndexEntries(dbTx database.Tx, block *fnoutil.Block, blockID uint32) error {
	...
	addEntries := func(txns []*fnoutil.Tx, txLocs []wire.TxLoc, blockID uint32) error {
		offset := 0
...
// block (if they were valid) and every stake transaction in the passed block.
func dbRemoveTxIndexEntries(dbTx database.Tx, block *fnoutil.Block) error {
	removeEntries := func(txns []*fnoutil.Tx) error {
		for _, tx := range txns {
...
//
// This is part of the Indexer interface.
func (idx *TxIndex) ConnectBlock(dbTx database.Tx, block, parent *fnoutil.Block, view *blockchain.UtxoViewpoint) error {
...
//
// This is part of the Indexer interface.
func (idx *TxIndex) DisconnectBlock(dbTx database.Tx, block, parent *fnoutil.Block, view *blockchain.UtxoViewpoint) error {
	// NOTE: The fact that the block can disapprove the regular tree of the
