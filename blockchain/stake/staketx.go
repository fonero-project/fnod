...
func AddrFromSStxPkScrCommitment(pkScript []byte,
	params *chaincfg.Params) (fnoutil.Address, error) {
	...
	if isP2SH {
		addr, err = fnoutil.NewAddressScriptHashFromHash(hashBytes, params)
	} else {
		addr, err = fnoutil.NewAddressPubKeyHash(hashBytes, params,
...
// ticket commitment pkScript.
func AmountFromSStxPkScrCommitment(pkScript []byte) (fnoutil.Amount, error) {
	if len(pkScript) < SStxPKHMinOutSize {
	...
	return fnoutil.Amount(binary.LittleEndian.Uint64(amtEncoded)), nil
}
