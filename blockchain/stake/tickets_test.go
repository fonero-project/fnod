...
import (
	...
	"github.com/fonero-project/fnod/blockchain/stake/internal/tickettreap"
...
func ticketsInBlock(bl *fnoutil.Block) []chainhash.Hash {
...
func TestTicketDBLongChain(t *testing.T) {
	...
	testBlockchain := make(map[int64]*fnoutil.Block, len(testBlockchainBytes))
	for k, v := range testBlockchainBytes {
		bl, err := fnoutil.NewBlockFromBytes(v)
		if err != nil {
