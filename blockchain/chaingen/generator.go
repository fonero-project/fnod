...
func (s *SpendableOut) Amount() fnoutil.Amount {
	return s.amount
}
...
func makeSpendableOutForTxInternal(tx *wire.MsgTx, blockHeight, txIndex, txOutIndex uint32, tree int8) SpendableOut {
	return SpendableOut{
		...
		blockIndex:  txIndex,
		amount:      fnoutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
type Generator struct {
	...
	p2shOpTrueAddr   fnoutil.Address
...
func MakeGenerator(params *chaincfg.Params) (Generator, error) {
	...
	p2shOpTrueAddr, err := fnoutil.NewAddressScriptHash(opTrueScript, params)
	if err != nil {
...
// a single OP_TRUE.
func (g *Generator) P2shOpTrueAddr() fnoutil.Address {
...
func (g *Generator) calcFullSubsidy(blockHeight uint32) fnoutil.Amount {
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcPoWSubsidy(fullSubsidy fnoutil.Amount, blockHeight uint32, numVotes uint16) fnoutil.Amount {
	powProportion := fnoutil.Amount(g.params.WorkRewardProportion)
	totalProportions := fnoutil.Amount(g.params.TotalSubsidyProportions())
	...
	ticketsPerBlock := fnoutil.Amount(g.params.TicketsPerBlock)
	return (powSubsidy * fnoutil.Amount(numVotes)) / ticketsPerBlock
}
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcPoSSubsidy(heightVotedOn uint32) fnoutil.Amount {
	if int64(heightVotedOn+1) < g.params.StakeValidationHeight {
		return 0
	}

	fullSubsidy := g.calcFullSubsidy(heightVotedOn)
	posProportion := fnoutil.Amount(g.params.StakeRewardProportion)
	totalProportions := fnoutil.Amount(g.params.TotalSubsidyProportions())
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcDevSubsidy(fullSubsidy fnoutil.Amount, blockHeight uint32, numVotes uint16) fnoutil.Amount {
	devProportion := fnoutil.Amount(g.params.BlockTaxProportion)
	totalProportions := fnoutil.Amount(g.params.TotalSubsidyProportions())
	...
	// Reduce the subsidy according to the number of votes.
	ticketsPerBlock := fnoutil.Amount(g.params.TicketsPerBlock)
	return (devSubsidy * fnoutil.Amount(numVotes)) / ticketsPerBlock
}
...
func (g *Generator) addCoinbaseTxOutputs(tx *wire.MsgTx, blockHeight uint32, devSubsidy, powSubsidy fnoutil.Amount) {
...
func PurchaseCommitmentScript(addr fnoutil.Address, amount, voteFeeLimit, revocationFeeLimit fnoutil.Amount) []byte {
...
func (g *Generator) CreateTicketPurchaseTx(spend *SpendableOut, ticketPrice, fee fnoutil.Amount) *wire.MsgTx {
...
func (g *Generator) CreateVoteTx(voteBlock *wire.MsgBlock, ticketTx *wire.MsgTx, ticketBlockHeight, ticketBlockIndex uint32) *wire.MsgTx {
	...
	voteSubsidy := posSubsidy / fnoutil.Amount(g.params.TicketsPerBlock)
	ticketPrice := fnoutil.Amount(ticketTx.TxOut[0].Value)
...
// children nodes.  A diagram depicting how this works for Fonero transactions
...
// will be nil.
func buildMerkleTreeStore(transactions []*fnoutil.Tx) []*chainhash.Hash {
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	utilTxns := make([]*fnoutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, fnoutil.NewTx(tx))
	}
...
//
// This compact form is only used in Fonero to encode unsigned 256-bit numbers
...
func (g *Generator) CreateSpendTx(spend *SpendableOut, fee fnoutil.Amount) *wire.MsgTx {
...
// opTrueRedeemScript.
func (g *Generator) CreateSpendTxForTx(tx *wire.MsgTx, blockHeight, txIndex uint32, fee fnoutil.Amount) *wire.MsgTx {
...
func (g *Generator) NextBlock(blockName string, spend *SpendableOut, ticketSpends []SpendableOut, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	...
	ticketPrice := fnoutil.Amount(g.CalcNextRequiredStakeDifficulty())
	...
	if nextHeight > uint32(g.params.CoinbaseMaturity) {
		...
		// outputs.
		if ticketSpends != nil {
			const ticketFee = fnoutil.Amount(2)
	...
	var stakeTreeFees fnoutil.Amount
	for _, tx := range stakeTxns {
		...
		var inputSum, outputSum fnoutil.Amount
		for _, txIn := range tx.TxIn {
			inputSum += fnoutil.Amount(txIn.ValueIn)
		}
		for _, txOut := range tx.TxOut {
			outputSum += fnoutil.Amount(txOut.Value)
		}
...
func (g *Generator) CreatePremineBlock(blockName string, additionalAmount fnoutil.Amount, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	...
	var totalSubsidy fnoutil.Amount
	for _, payout := range g.params.BlockOneLedger {
		payoutAddr, err := fnoutil.DecodeAddress(payout.Address)
		if err != nil {
			panic(err)
		}
		...
		totalSubsidy += fnoutil.Amount(payout.Amount)
	}
