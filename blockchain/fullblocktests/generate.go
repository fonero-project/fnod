...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := fnoutil.Hash160(redeemScript)
...
// subsidy.
func additionalCoinbasePoW(amount fnoutil.Amount) func(*wire.MsgBlock) {
...
func additionalCoinbaseDev(amount fnoutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
		// Increase the first proof-of-work coinbase subsidy by the
		// provided amount.
...
func additionalSpendFee(fee fnoutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
func Generate(includeLargeReorg bool) (tests [][]TestInstance, err error) {
	...
	g.NextBlock("bbadtaxscript", outs[5], ticketOuts[5],
		func(b *wire.MsgBlock) {
			...
			p2shTaxAddr := addrs[0].(*fnoutil.AddressScriptHash)
			p2pkhTaxAddr, err := fnoutil.NewAddressPubKeyHash(
	...
	g.NextBlock("bv15", outs[9], ticketOuts[9], func(b *wire.MsgBlock) {
		ticketFee := fnoutil.Amount(2)
		ticketPrice := fnoutil.Amount(g.CalcNextReqStakeDifficulty(g.Tip()))
		ticketPrice--
	...
	// vector is not possible in Fonero since the block size is included in
	...
	// Fonero enforces the coinbase includes the block height to which it
	...
	g.NextBlock("bcb9", outs[19], ticketOuts[19], func(b *wire.MsgBlock) {
		b.Transactions[1].TxOut[0].Value = fnoutil.MaxAmount + 1
	})
	rejected(blockchain.ErrBadTxOutValue)
	...
	g.NextBlock("bcb10", outs[19], ticketOuts[19], func(b *wire.MsgBlock) {
		b.Transactions[1].TxOut[0].Value = fnoutil.MaxAmount
