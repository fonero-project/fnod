...
func IsCoinBase(tx *fnoutil.Tx) bool {
...
// level util transaction as opposed to a raw wire transaction.
func IsExpired(tx *fnoutil.Tx, blockHeight int64) bool {
...
func IsFinalizedTransaction(tx *fnoutil.Tx, blockHeight int64, blockTime time.Time) bool {
...
func CheckTransactionSanity(tx *wire.MsgTx, params *chaincfg.Params) error {
	...
	// Fonero is a quantity of atoms as defined by the AtomsPerCoin constant.
	//
...
func checkProofOfStake(block *fnoutil.Block, posLimit int64) error {
...
func checkBlockSanity(block *fnoutil.Block, timeSource MedianTimeSource, flags BehaviorFlags, chainParams *chaincfg.Params) error {
	msgBlock := block.MsgBlock()
...
// sane before continuing with block processing.  These checks are context
// free.
func CheckBlockSanity(block *fnoutil.Block, timeSource MedianTimeSource, chainParams *chaincfg.Params) error {
...
func (b *BlockChain) checkBlockPositional(block *fnoutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
	// The genesis block is valid by definition.
	if prevNode == nil {
		return nil
	}
...
// impossible.
func checkCoinbaseUniqueHeight(blockHeight int64, block *fnoutil.Block) error {
...
func (b *BlockChain) checkBlockContext(block *fnoutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
	// The genesis block is valid by definition.
	if prevNode == nil {
		return nil
	}
...
//
// Fonero: Check the stake transactions to make sure they don't have this txid
// too.
func (b *BlockChain) checkDupTxs(txSet []*fnoutil.Tx, view *UtxoViewpoint) error {
...
// NOTE: The caller MUST have already determined that the provided transaction
// is a vote.
func checkVoteInputs(subsidyCache *SubsidyCache, tx *fnoutil.Tx, txHeight int64, view *UtxoViewpoint, params *chaincfg.Params) error {
...
// NOTE: The caller MUST have already determined that the provided transaction
// is a revocation.
func checkRevocationInputs(tx *fnoutil.Tx, txHeight int64, view *UtxoViewpoint, params *chaincfg.Params) error {
	ticketMaturity := int64(params.TicketMaturity)
	revokeHash := tx.Hash()
	msgTx := tx.MsgTx()
...
// owner of the Fonero and therefore allowed to spend them.  As it checks the
// inputs, it also calculates the total fees for the transaction and returns
// that value.
//
// NOTE: The transaction MUST have already been sanity checked with the
// CheckTransactionSanity function prior to calling this function.
func CheckTransactionInputs(subsidyCache *SubsidyCache, tx *fnoutil.Tx, txHeight int64, view *UtxoViewpoint, checkFraudProof bool, chainParams *chaincfg.Params) (int64, error) {
	...
	// -------------------------------------------------------------------
	// Fonero stake transaction testing.
	// -------------------------------------------------------------------
	...
	// -------------------------------------------------------------------
	// Fonero general transaction testing (and a few stake exceptions).
	// -------------------------------------------------------------------
	...
	for idx, txIn := range msgTx.TxIn {
		...
		// Fonero is a quantity of atoms as defined by the AtomPerCoin
		// constant.
		...
		if originTxAtom > fnoutil.MaxAmount {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				originTxAtom, fnoutil.MaxAmount)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
...
// txscript.
func CountSigOps(tx *fnoutil.Tx, isCoinBaseTx bool, isSSGen bool) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *fnoutil.Tx, isCoinBaseTx bool, isStakeBaseTx bool, view *UtxoViewpoint) (int, error) {
...
func (b *BlockChain) createLegacySeqLockView(block, parent *fnoutil.Block, view *UtxoViewpoint) (*UtxoViewpoint, error) {
...
func checkNumSigOps(tx *fnoutil.Tx, view *UtxoViewpoint, index int, txTree bool, cumulativeSigOps int) (int, error) {
	msgTx := tx.MsgTx()
	isSSGen := stake.IsSSGen(msgTx)
...
func checkStakeBaseAmounts(subsidyCache *SubsidyCache, height int64, params *chaincfg.Params, txs []*fnoutil.Tx, view *UtxoViewpoint) error {
	for _, tx := range txs {
		msgTx := tx.MsgTx()
...
func getStakeBaseAmounts(txs []*fnoutil.Tx, view *UtxoViewpoint) (int64, error) {
	totalInputs := int64(0)
	totalOutputs := int64(0)
	for _, tx := range txs {
		msgTx := tx.MsgTx()
...
func getStakeTreeFees(subsidyCache *SubsidyCache, height int64, params *chaincfg.Params, txs []*fnoutil.Tx, view *UtxoViewpoint) (fnoutil.Amount, error) {
	totalInputs := int64(0)
	totalOutputs := int64(0)
	for _, tx := range txs {
		msgTx := tx.MsgTx()
	...
	return fnoutil.Amount(totalInputs - totalOutputs), nil
}
...
func (b *BlockChain) checkTransactionsAndConnect(subsidyCache *SubsidyCache, inputFees fnoutil.Amount, node *blockNode, txs []*fnoutil.Tx, view *UtxoViewpoint, stxos *[]spentTxOut, txTree bool) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block, parent *fnoutil.Block, view *UtxoViewpoint, stxos *[]spentTxOut) error {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) CheckConnectBlockTemplate(block *fnoutil.Block) error {
	b.chainLock.Lock()
