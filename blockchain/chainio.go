...
import (
	...
	"github.com/fonero-project/fnod/blockchain/internal/dbnamespace"
...
func serializeSizeForMinimalOutputs(tx *fnoutil.Tx) int {
...
func putTxToMinimalOutputs(target []byte, tx *fnoutil.Tx) int {
...
// already there.
func dbMaybeStoreBlock(dbTx database.Tx, block *fnoutil.Block) error {
...
// The amount is never encoded into spent transaction outputs in Fonero
...
func decodeSpentTxOut(serialized []byte, stxo *spentTxOut, amount int64, height uint32, index uint32) (int, error) {
	...
	// since in Fonero we only need pkScript at most due to fraud proofs
...
// txouts.
func dbFetchSpendJournalEntry(dbTx database.Tx, block *fnoutil.Block) ([]spentTxOut, error) {
...
func (b *BlockChain) createChainState() error {
	...
	genesisBlock := fnoutil.NewBlock(b.chainParams.GenesisBlock)
...
// block for the provided node, deserialize it, and return a fnoutil.Block.
func dbFetchBlockByNode(dbTx database.Tx, node *blockNode) (*fnoutil.Block, error) {
	...
	block, err := fnoutil.NewBlockFromBytes(blockBytes)
	if err != nil {
		return nil, err
	}
