...
// BlockChain provides functions for working with the Fonero block chain.
...
type BlockChain struct {
	...
	mainchainBlockCache     map[chainhash.Hash]*fnoutil.Block
...
// StakeVersions is a condensed form of a fnoutil.Block that is used to prevent
...
// exceeded.
func (b *BlockChain) addOrphanBlock(block *fnoutil.Block) {
...
func (b *BlockChain) fetchMainChainBlockByNode(node *blockNode) (*fnoutil.Block, error) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) fetchBlockByNode(node *blockNode) (*fnoutil.Block, error) {
...
func (b *BlockChain) pushMainChainBlockCache(block *fnoutil.Block) {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBlock(node *blockNode, block, parent *fnoutil.Block, view *UtxoViewpoint, stxos []spentTxOut) error {
	...
	blockAndParent := []*fnoutil.Block{block, parent}
...
func (b *BlockChain) dropMainChainBlockCache(block *fnoutil.Block) {
	curHash := block.Hash()
	b.mainchainBlockCacheLock.Lock()
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) disconnectBlock(node *blockNode, block, parent *fnoutil.Block, view *UtxoViewpoint) error {
...
func countSpentRegularOutputs(block *fnoutil.Block) int {
...
func countSpentStakeOutputs(block *fnoutil.Block) int {
	var numSpent int
...
func countSpentOutputs(block *fnoutil.Block) int {
...
func (b *BlockChain) reorganizeChainInternal(targetTip *blockNode) error {
	...
	var nextBlockToDetach *fnoutil.Block
	...
	var prevBlockAttached *fnoutil.Block
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBestChain(node *blockNode, block, parent *fnoutil.Block, flags BehaviorFlags) (int64, error) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockByHash(hash *chainhash.Hash) (*fnoutil.Block, error) {
	node := b.index.LookupNode(hash)
...
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockByHeight(height int64) (*fnoutil.Block, error) {
...
type IndexManager interface {
	...
	// main chain.
	ConnectBlock(database.Tx, *fnoutil.Block, *fnoutil.Block, *UtxoViewpoint) error

	// DisconnectBlock is invoked when a block has been disconnected from
	// the main chain.
	DisconnectBlock(database.Tx, *fnoutil.Block, *fnoutil.Block, *UtxoViewpoint) error
}
...
func New(config *Config) (*BlockChain, error) {
	...
	b := BlockChain{
		...
		mainchainBlockCache:           make(map[chainhash.Hash]*fnoutil.Block),
