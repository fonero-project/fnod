...
// FileContents is a string containing the commented example config for fnod.
...
; The default is ~/.fnod/data on POSIX OSes, $LOCALAPPDATA/Dcrd/data on Windows,
; ~/Library/Application Support/Dcrd/data on macOS, and $homed/fnod/data on
; Plan9.  Environment variables are expanded so they may be used.  NOTE: Windows
; environment variables are typically %VARIABLE%, but they must be accessed with
; $VARIABLE here.
; datadir=~/.fnod/data                            ; Unix
...
; line.  fnod will not contact 3rd-party sites to obtain external ip addresses.
...
; with, but the behavior is slightly different.  By default, fnod will query DNS
...
; Disable DNS seeding for peers.  By default, when fnod starts, it will use
...
; which is used to control and query information from a running fnod process.
;
...
; log level for individual subsystems.  Use fnod --debuglevel=show to list
