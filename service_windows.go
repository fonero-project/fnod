...
const (
	// svcName is the name of fnod service.
	svcName = "fnodsvc"
	...
	svcDesc = "Downloads and stays synchronized with the Fonero block " +
...
// logServiceStartOfDay logs information about fnod when the main server has
...
// fnodService houses the main service handler which handles all service
// updates and launching fnodMain.
type fnodService struct{}
...
// long-running fnodMain (which is the real meat of fnod), handles service
// change requests, and notifies the service control manager of changes.
func (s *fnodService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start fnodMain in a separate goroutine so the service can start
...
// installService attempts to install the fnod service.  Typically this should
...
// removeService attempts to uninstall the fnod service.  Typically this should
...
// startService attempts to start the fnod service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &fnodService{})
	if err != nil {
