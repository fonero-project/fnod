...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt fnoutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(fnoutil.Amount(5 * fnoutil.AtomsPerCoin))
	...
	txid = genSpend(fnoutil.Amount(5000 * fnoutil.AtomsPerCoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testJoinMempools(r *Harness, t *testing.T) {
	...
	pooledHashes, err := r.Node.GetRawMempool(fnojson.GRMAll)
	if err != nil {
	...
	go func() {
		for {
			poolHashes, err := r.Node.GetRawMempool(fnojson.GRMAll)
			if err != nil {
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := fnoutil.Amount(5 * 300 * fnoutil.AtomsPerCoin)
	...
	// chain.
	expectedBalance = fnoutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := fnoutil.Amount(50 * fnoutil.AtomsPerCoin)
...
func TestHarness(t *testing.T) {
	...
	expectedBalance := fnoutil.Amount(numMatureOutputs * 300 * fnoutil.AtomsPerCoin)
