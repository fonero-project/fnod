...
type memWallet struct {
	...
	addrs map[uint32]fnoutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]fnoutil.Address)
...
// SetRPCClient saves the passed rpc connection to fnod as the wallet's
...
func (m *memWallet) IngestBlock(header []byte, filteredTxns [][]byte) {
	...
	txns := make([]*fnoutil.Tx, 0, len(filteredTxns))
	for _, txBytes := range filteredTxns {
		tx, err := fnoutil.NewTxFromBytes(txBytes)
		if err != nil {
			panic(err)
		}
		txns = append(txns, tx)
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash, isCoinbase bool, undo *undoEntry) {
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          fnoutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (fnoutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []fnoutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt fnoutil.Amount, feeRate fnoutil.Amount) error {
	const (
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := fnoutil.Amount(txSize * int(feeRate))
...
// in atoms-per-byte.
func (m *memWallet) SendOutputs(outputs []*wire.TxOut, feeRate fnoutil.Amount) (*chainhash.Hash, error) {
...
//
// This function is safe for concurrent access.
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut, feeRate fnoutil.Amount) (*wire.MsgTx, error) {
	m.Lock()
	defer m.Unlock()
	...
	var outputAmt fnoutil.Amount
	for _, output := range outputs {
		outputAmt += fnoutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() fnoutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance fnoutil.Amount
...
func keyToAddr(key *secp256k1.PrivateKey, net *chaincfg.Params) (fnoutil.Address, error) {
	pubKey := (*secp256k1.PublicKey)(&key.PublicKey)
	serializedKey := pubKey.SerializeCompressed()
	pubKeyAddr, err := fnoutil.NewAddressSecpPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
