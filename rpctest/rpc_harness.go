...
// Harness fully encapsulates an active fnod process to provide a unified
// platform for creating rpc driven integration tests involving fnod. The
// active fnod node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active fnod process is fully
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	...
	// wallet.
	filterAddrs := []fnoutil.Address{h.wallet.coinbaseAddr}
	if err := h.Node.LoadTxFilter(true, filterAddrs, nil); err != nil {
		return err
	}

	// Ensure fnod properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created fnod
...
func (h *Harness) NewAddress() (fnoutil.Address, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) ConfirmedBalance() fnoutil.Amount {
...
//
// This function is safe for concurrent access.
func (h *Harness) SendOutputs(targetOutputs []*wire.TxOut, feeRate fnoutil.Amount) (*chainhash.Hash, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) CreateTransaction(targetOutputs []*wire.TxOut, feeRate fnoutil.Amount) (*wire.MsgTx, error) {
