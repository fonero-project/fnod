...
// newAddressPubKey returns a new fnoutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) fnoutil.Address {
	pubkey, err := secp256k1.ParsePubKey(serializedPubKey)
	if err != nil {
		panic("invalid public key in test source")
	}
	addr, err := fnoutil.NewAddressSecpPubKeyCompressed(pubkey,
...
// newAddressPubKeyHash returns a new fnoutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) fnoutil.Address {
	addr, err := fnoutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams,
...
// newAddressScriptHash returns a new fnoutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) fnoutil.Address {
	addr, err := fnoutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the fnoutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the fnoutil.Address interface.
...
// DSA returns -1.
func (b *bogusAddress) DSA(chainParams *chaincfg.Params) fnoec.SignatureType {
	return -1
}
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := fnoutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := fnoutil.NewAddressScriptHashFromHash(hexToBytes("e8c30"+
	...
	p2pkCompressedMain, err := fnoutil.NewAddressSecpPubKey(hexToBytes("02192d7"+
	...
	p2pkCompressed2Main, err := fnoutil.NewAddressSecpPubKey(hexToBytes("03b0b"+
	...
	tests := []struct {
		...
		// pay-to-pubkey address on mainnet. for Fonero this would
		// be uncompressed, but standard for Fonero is 33 byte
		...
		{(*fnoutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*fnoutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*fnoutil.AddressSecpPubKey)(nil), "", errUnsupportedAddress},
		{(*fnoutil.AddressEdwardsPubKey)(nil), "", errUnsupportedAddress},
		{(*fnoutil.AddressSecSchnorrPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	//  mainnet p2pk 13CG6SJ3yHUXo4Cr2RY4THLLJrNFuG3gUg
	p2pkCompressedMain, err := fnoutil.NewAddressSecpPubKey(hexToBytes("02192d"+
	...
	tests := []struct {
		keys      []*fnoutil.AddressSecpPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*fnoutil.AddressSecpPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
		...
		{
			// By default compressed pubkeys are used in Fonero.
			[]*fnoutil.AddressSecpPubKey{
				p2pkUncompressedMain.(*fnoutil.AddressSecpPubKey),
			},
			1,
...
func TestGenerateSStxAddrPush(t *testing.T) {
	...
	for _, test := range tests {
		addr, err := fnoutil.DecodeAddress(test.addrStr)
		if err != nil {
