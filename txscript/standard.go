...
func payToPubKeyHashEdwardsScript(pubKeyHash []byte) ([]byte, error) {
	edwardsData := []byte{byte(fnoec.STEd25519)}
	return NewScriptBuilder().AddOp(OP_DUP).AddOp(OP_HASH160).
...
func payToPubKeyHashSchnorrScript(pubKeyHash []byte) ([]byte, error) {
	schnorrData := []byte{byte(fnoec.STSchnorrSecp256k1)}
	return NewScriptBuilder().AddOp(OP_DUP).AddOp(OP_HASH160).
...
// valid SStxs.
func PayToSStx(addr fnoutil.Address) ([]byte, error) {
	...
	case *fnoutil.AddressPubKeyHash:
		...
		if addr.DSA(addr.Net()) != fnoec.STEcdsaSecp256k1 {
...
// valid SStxs.
func PayToSStxChange(addr fnoutil.Address) ([]byte, error) {
	// Only pay to pubkey hash and pay to script hash are
	// supported.
	scriptType := PubKeyHashTy
	switch addr := addr.(type) {
...
// valid SSGen txs.
func PayToSSGen(addr fnoutil.Address) ([]byte, error) {
	// Only pay to pubkey hash and pay to script hash are
	// supported.
	scriptType := PubKeyHashTy
	switch addr := addr.(type) {
...
// valid SSRtx.
func PayToSSRtx(addr fnoutil.Address) ([]byte, error) {
	// Only pay to pubkey hash and pay to script hash are
	// supported.
	scriptType := PubKeyHashTy
	switch addr := addr.(type) {
...
// an SStx.
func GenerateSStxAddrPush(addr fnoutil.Address, amount fnoutil.Amount, limits uint16) ([]byte, error) {
	// Only pay to pubkey hash and pay to script hash are
	// supported.
	scriptType := PubKeyHashTy
	switch addr := addr.(type) {
...
// specified address.
func PayToAddrScript(addr fnoutil.Address) ([]byte, error) {
	switch addr := addr.(type) {
	case *fnoutil.AddressPubKeyHash:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
	...
	case *fnoutil.AddressEdwardsPubKey:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
				nilAddrErrStr)
		}
	...
	case *fnoutil.AddressSecSchnorrPubKey:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
				nilAddrErrStr)
		}
...
func MultiSigScript(pubkeys []*fnoutil.AddressSecpPubKey, nrequired int) ([]byte, error) {
...
func ExtractPkScriptAddrs(version uint16, pkScript []byte,
	chainParams *chaincfg.Params) (ScriptClass, []fnoutil.Address, int, error) {
	if version != DefaultScriptVersion {
	...
	case PubKeyHashTy:
		...
		requiredSigs = 1
		addr, err := fnoutil.NewAddressPubKeyHash(pops[2].data,
			chainParams, fnoec.STEcdsaSecp256k1)
		if err == nil {
	...
	case PubKeyTy:
		...
		if err == nil {
			addr, err := fnoutil.NewAddressSecpPubKeyCompressed(pk, chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
		}
	...
	case PubkeyAltTy:
		...
		switch suite {
		case fnoec.STEd25519:
			addr, err = fnoutil.NewAddressEdwardsPubKey(pops[0].data,
				chainParams)
		case fnoec.STSchnorrSecp256k1:
			addr, err = fnoutil.NewAddressSecSchnorrPubKey(pops[0].data,
				chainParams)
		}
		if err == nil {
			addrs = append(addrs, addr)
	...
	case StakeSubmissionTy:
		...
		var localAddrs []fnoutil.Address
	...
	case ScriptHashTy:
		...
		requiredSigs = 1
		addr, err := fnoutil.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case MultiSigTy:
		...
		addrs = make([]fnoutil.Address, 0, numPubKeys)
...
func ExtractPkScriptAltSigType(pkScript []byte) (fnoec.SignatureType, error) {
	pops, err := parseScript(pkScript)
	if err != nil {
		return 0, err
	}
	...
	val := fnoec.SignatureType(valInt)
	switch val {
	case fnoec.STEd25519:
		return val, nil
	case fnoec.STSchnorrSecp256k1:
...
//
// NOTE: Atomic swaps are not considered standard script types by the fnod
