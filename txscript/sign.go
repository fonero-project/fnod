...
func RawTxInSignatureAlt(tx *wire.MsgTx, idx int, subScript []byte,
	hashType SigHashType, key chainec.PrivateKey, sigType fnoec.SignatureType) ([]byte,
	error) {
...
func SignatureScriptAlt(tx *wire.MsgTx, idx int, subscript []byte,
	...
	sigType fnoec.SignatureType) ([]byte,
	error) {
...
func p2pkSignatureScriptAlt(tx *wire.MsgTx, idx int, subScript []byte,
	hashType SigHashType, privKey chainec.PrivateKey, sigType fnoec.SignatureType) ([]byte,
	error) {
	sig, err := RawTxInSignatureAlt(tx, idx, subScript, hashType, privKey,
		sigType)
	if err != nil {
...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType,
	addresses []fnoutil.Address, nRequired int, kdb KeyDB) ([]byte, bool) {
	// No need to add dummy in Fonero.
...
func handleStakeOutSign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	addresses []fnoutil.Address, class ScriptClass, subClass ScriptClass,
	nrequired int) ([]byte, ScriptClass, []fnoutil.Address, int, error) {
...
func sign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	subScript []byte, hashType SigHashType, kdb KeyDB, sdb ScriptDB,
	sigType fnoec.SignatureType) ([]byte,
	ScriptClass, []fnoutil.Address, int, error) {
...
func mergeScripts(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	pkScript []byte, class ScriptClass, addresses []fnoutil.Address,
...
func mergeMultiSig(tx *wire.MsgTx, idx int, addresses []fnoutil.Address,
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		if err != nil {
			// Fonero -- is this the right handling for SIGHASH_SINGLE error ?
		...
		for _, addr := range addresses {
			...
			// bad input.
			pkaddr := addr.(*fnoutil.AddressSecpPubKey)
...
type KeyDB interface {
	GetKey(fnoutil.Address) (chainec.PrivateKey, bool, error)
}

// KeyClosure implements KeyDB with a closure.
type KeyClosure func(fnoutil.Address) (chainec.PrivateKey, bool, error)

// GetKey implements KeyDB by returning the result of calling the closure.
func (kc KeyClosure) GetKey(address fnoutil.Address) (chainec.PrivateKey,
	bool, error) {
	return kc(address)
}
...
type ScriptDB interface {
	GetScript(fnoutil.Address) ([]byte, error)
}

// ScriptClosure implements ScriptDB with a closure.
type ScriptClosure func(fnoutil.Address) ([]byte, error)

// GetScript implements ScriptDB by returning the result of calling the closure.
func (sc ScriptClosure) GetScript(address fnoutil.Address) ([]byte, error) {
	return sc(address)
}
...
func SignTxOutput(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	previousScript []byte, sigType fnoec.SignatureType) ([]byte, error) {
