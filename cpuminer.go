...
type cpuminerConfig struct {
	...
	ProcessBlock func(*fnoutil.Block, blockchain.BehaviorFlags) (bool, error)
...
func (m *CPUMiner) submitBlock(block *fnoutil.Block) bool {
	...
	minrLog.Infof("Block submitted via CPU miner accepted (hash %s, "+
		...
		fnoutil.Amount(coinbaseTxGenerated))
	return true
}
...
out:
	for {
		...
		if m.solveBlock(template.Block, ticker, quit) {
			block := fnoutil.NewBlock(template.Block)
			m.submitBlock(block)
